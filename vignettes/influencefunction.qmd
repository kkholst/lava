...
Direct calculations is also possible, if `a` and `b` are estimate objects we can
...
Let $O_1 = (Z_1, R_1), \ldots, O_N = (Z_1, R_N)$ be iid data from some
...
\leq
P(E_N^\complement)
+ \sup_{n>Np/2} Q(\lVert Z_n \rVert > \epsilon/c)
\end{align*}
...
\begin{align*}
P(N-n - N(1-p) \geq t) \leq \exp(-2t^2/N) \to 0 \text{ as }N\to\infty,
\end{align*}
and with $t = Np/2$ it follows that the first term $P(E_N^\complement)$ also
converges to zero.
...
(\frac{1}{\widehat{p}}-\frac{1}{\widehat{p}})
...
the joint parameter corresponds to o an inverse probability
weighting with the empirical plugin estimate of the proportion
$R_{1}=1$, and $R_{2}=1$
\begin{align*}
\sqrt{N}\left\{
\begin{pmatrix}
...
In the above example we also demonstrated the user of the subset operator `[]`
...
$\Pz$ and $P_{n}$ are here everywhere the integrals wrt. $X$. Then the influence
