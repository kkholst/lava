...
curve [@bickel_effic_adapt_estim_semip_model; @vaart_1998_asymp]. The IF allows for the direct estimation of properties of the estimator, including its asymptotic variance. Moreover,
...
\Pz)\}=0$ and $\var\{\IC(Z_{i}; \Pz)\}<\infty$ [@tsiatis2006semiparametric;
...
in the sense that there exists a bounded linear functional  $\dot\Psi\colon
...
- `IC`, `coef`, `vcov` methods to extract the estimated IF, coefficients, and
...
specified using the `coef` and `IC` arguments. If the primary goal is to apply the
delta method or test linear hypotheses, it is also possible to provide the
asymptotic variance estimate via the `vcov` argument, without specifying the IC
matrix.
...
```{r estimate.examples, eval=FALSE}
...
Direct calculations are also possible, if `a` and `b` are estimate objects we can
obtain new transformed parameter estimates with the syntax
```{r estimate.direct, eval=FALSE}
c(exp(b)^0.5, exp(b * a) / (1 + exp(-b)))
```
with the necessary derivatives exactly calculated automatically.
We refer to section
[IF building blocks](#if-building-blocks-transformations-and-the-delta-theorem) for more details.
...
estimate(g)
```
We can compare that to the usual (non-robust) standard errors by supplying
the model-based covariance matrix explicitly:
```{r glm.std}
...
\begin{align*}
\log\left(\frac{\pr(Y\leq j\mid x)}{1-\pr(Y\leq j\mid x)}\right) = \operatorname{expit}(\alpha_{j} - \beta^{\top}x), \quad j=1,\ldots,J-1
\end{align*}
```{r ordreg}
ordreg(y1 ~ a + x1, dw, family=binomial(logit)) |> estimate()
```

Note that the `sandwich::estfun` function from the `sandwich` library
[@r_sandwich] can also be used to estimate the IF
...
The `estimate` and `IC` methods are also available for parametric survival
...
\IC_m(Z_i; \Pz) + \op(1)
\end{align*}
based on iid data $Z_1,\ldots,Z_n$. Then
the joint estimate $\widehat{\theta} =
(\widehat{\theta}_{1}^{\top},\ldots,\widehat{\theta}_{M}^{\top})^\top$
of
${\theta} = ({\theta}_{1}^{\top},\ldots,{\theta}_{M}^{\top})^\top$ has influence
function given by
\begin{align*}
\overline{\IC}(Z_i; \Pz) = [\IC_{1}(Z_i; \Pz)^\top,\ldots,\IC_{M}(Z_i; \Pz)^\top]^{\top}.
\end{align*}
\flushright$\blacksquare$
::::
This follows immediately from [@vaart_1998_asymp, Theorem 18.10[vi]], and thus
\begin{align*}
...
\end{align*}
by the CLT, and under regularity conditions
...
```
More details can be found in the Section on [hypothesis testing](#linear-contrasts-and-hypothesis-testing).

## Estimators computed on different subsets

Let $O_1 = (Z_1, R_1), \ldots, O_N = (Z_N, R_N)$ be iid data from some
distribution $O = (Z, R) \sim P_0$. We let $Q_0 := P_0(\cdot \mid R=1)$ denote the
conditional distribution, with $P(R=1) = p > 0$, and let $S = \{i\mid R_i=1\}$,
$n = \lvert S\rvert$, and $\widehat{p} = n/N$.

We assume that the *conditional* target parameter $\theta = T(Q_0)$ is pathwise differentiable in $Q_0$, in the sense that
we have a regular and asymptotic linear estimator $\widehat{\theta} = T_m(O_1, \ldots, O_m)$
based on iid data $O_1,\ldots,O_m \sim Q_0$,
...
\big\rVert > \eta
\Big) \longrightarrow 0 \text{ as } m\to\infty.
\end{align*}

:::: {#lem-conditional}
The estimator $\widehat{\theta}$ has influence function
$\widetilde{\IC}(O; Q_0) = \frac{R}{p}\IC(O; Q_0)$, i.e.
\begin{align*}
\sqrt{N}(\widehat{\theta} - \theta) =
\frac{1}{p}\frac{1}{\sqrt{N}}\sum_{i=1}^N
R_i \IC(O_i; Q_0) + o_{P_0}(1).
\end{align*}
::::
...
We can write
\begin{align*}
\sqrt{N}(\widehat{\theta}-\theta) = \frac{1}{\widehat{p}}\frac{1}{\sqrt{N}}
\sum_{i=1}^N R_i \IC(O_i; Q_0) +
\underbrace{\sqrt{N}\left(
\widehat{\theta}-\theta-\frac{1}{n}\sum_{i\in S}\IC(O_i; Q_0)
\right)}_{W_N = \sqrt{N/n}V_n}.
\end{align*}

We need to show that the term $W_N=o_{P_0}(1)$.
Let $E_N = \{n > Np/2\}$, it then follows that
\begin{align*}
P(\lVert W_N \rVert > \epsilon) \leq P(E_N^\complement)
+ P(\{\lVert W_N \rVert > \epsilon\}\cap E_N) \\
\leq
P(E_N^\complement)
+ \sup_{n>Np/2} Q(\lVert V_n \rVert > \epsilon/c)
\end{align*}
where we have used that on the set $E_N$ we have $\sqrt{N/n}< \sqrt{2/p} = c$.
The supremum term converges to zero as $N\to\infty$ due to the assumption that
$\widehat{\theta}$ is RAL in $Q_0$.

Applying Hoeffding's Inequality to the iid Bernoulli random variables $1-R_i,
i=1,\ldots,N$, we get that
\begin{align*}
P(N-n - N(1-p) \geq t) \leq \exp(-2t^2/N),
\end{align*}
and with $t = Np/2$, we have that $P(n<Np/2)\leq\exp(-N/(2p^2))\to 0$ as
$N\to\infty$, and it follows that the first term $P(E_N^\complement)$ also
converges to zero.

As $\widehat{p}\to p$ a.s. by the strong LLN, it follows from continuity at
$p>0$ that $\widehat{p}^{-1}-p^{-1} = o_{P_0}(1)$, and hence by Slutsky's lemma
\begin{align*}
\sqrt{N}(\widehat{\theta}-\theta) &= \frac{1}{\sqrt{N}}\sum_{i=1}^N
\frac{R_i}{p}\IC(O_i; Q_0) +
(\frac{1}{\widehat{p}}-\frac{1}{p})
\frac{1}{\sqrt{N}}\sum_{i=1}^N R_i\IC(O_i; Q_0) +
o_{P_0}(1) \\
&= \frac{1}{\sqrt{N}}\sum_{i=1}^N
\frac{R_i}{p}\IC(O_i; Q_0) + o_{P_0}(1).
\end{align*}
\flushright$\blacksquare$
:::


Based on the above Lemma we can therefore combine estimators that are estimated
on different datasets. Let the observed data be
$(Z_{11}, R_{11}), (Z_{21}, R_{21}), \ldots, (Z_{1N}, R_{1N}),
(Z_{2N}, R_{2N})$ with estimators
$\widehat{\theta}_1$ and
$\widehat{\theta}_2$ for the conditional parameters $\theta_1$ and
$\theta_2$ based on data where $R_{1i}=1$ and $R_{2j}=1$, respectively, and let the
corresponding IFs be
$IC_{1}(\cdot; \Pz)$ and $IC_{2}(\cdot;\Pz)$. It then follows that
the joint parameter estimate has influence function corresponding to an inverse probability
weighting with the empirical plugin estimate of the proportions
$P(R_{1}=1)$, and $P(R_{2}=1)$
\begin{align*}
\sqrt{N}\left\{
\begin{pmatrix}
\widehat{\theta}_1 \\
...
An often more convenient syntax is to directly transform the `estimate` objects
via standard mathematical R operations. An advantage of this is also that the
necessary derivatives are automatically calculated exactly.
To illustrate this consider the following simple example where we consider two
`estimate` objects each with a single parameter estimate
...
Parameter transformation can now be calculated directly as in the following examples
```{r estimate_calc_ex}
a * b
(3 * cos(a) / sqrt(b) + 1) / a
e <- c(a,b) # merge
...
(see also Section on [Linear contrasts](#linear-contrasts-and-hypothesis-testing))
```{r estimate-contrast}
...
and any compositions of these functions, which also means we can immediately
 apply most existing user-defined functions. To give an example consider the `logit` function
```{r}
lava::logit
logit(b)
expit(c(a,b))
...
and the merge method `c()`. These are basically equivalent to the `subset` and
`merge` methods. There is also a `transform` method for constructing parameter
transformations above, and the `labels` function for renaming parameters.
Together we can combine these methods via the pipe operator to create readable
code:
```{r estimate-pipe}
...
\begin{align*}
\widehat{\cov}(X_1,X_2) = \mathbb{P}_n(X_1-\mathbb{P}_n X_1)(X_2-\mathbb{P}_n X_2).
\end{align*}
...
```
The variance estimates can be estimated in the same way and combined to estimate the correlation
```{r rho}
...
back to the original correlation scale via the `transform` argument of the `summary` method.
...
We refer to the function `lava::contr`  and `lava::parsedesign` for defining contrast matrices.
```{r contr}
...
rate (FWER) under the null can be approximated by
$$
P(Z_{max} > z) =
1-\int_{-z}^{z} \cdots \int_{-z}^{z} \phi_{R}(x_{1},\ldots,x_{p})
\,dx_{1}\cdots\,dx_{p}
$$
where $\phi_{R}$ is the multivariate normal density
function with mean 0 and variance given by the correlation matrix $R$. The
adjusted $p$-values can then be calculated as
$$
P(Z_{max} >
\Phi^{-1}(1-p/2))
$$
where $\Phi$ is the standard Gaussian CDF. As described in
in [@RitzPipper_multcomp] the joint distribution of $Z_{1},\ldots,Z_{p}$
can be estimated from the IFs. This is implemented in the `alpha_zmax` method
...
adjustments, another powerful closed-testing procedure [@marcus1976], can be generally
obtained by considering all intersection hypotheses.

![*Figure: closed testing via Wald tests of all intersections hypotheses.*](../man/figures/closedtesting.svg){width=60%}
...
Let $\Psi = \Pz f(X;\theta)$ and $\widehat{\Psi} = P_{n} f(X;\widehat{\theta})$.
$\Pz$ and $P_{n}$ are here denoting the integrals wrt. $X$. Then the influence
function for $\widehat{\Psi}$ is given by
\begin{align*}
IC(Z; \Pz) = f(X;\theta)-\Psi + [\Pz\nabla_{\theta}f(X;\theta)]\phi(Z).
\end{align*}
...
gives the predicted probability $\pr(Y=1\mid A=1,X_{1},W)$ for any observed
...
Let the observed data be $Z = (W,A,Y)$, where $W$ are covariates, $A$ a binary treatment
variable, and $Y$ the outcome.
...
treatment effect we can then consider the *average treatment effect*
...
2)  Positivity, $P(A=a\mid W)>\epsilon$ for some $\epsilon>0$ and baseline
    covariates $W$
3)  No unmeasured confounders, $Y(a)\indep A|W$
...
To illustrate this we manually construct the above estimating equations based on
logistic regressions models for the two nuisance models
```{r ate-nuisancemodels}
...
We can then easily construct estimates of the two expected potential outcomes
and estimate their joint asymptotic distribution
```{r est-potentialoutcomes}
...
Finally, we can obtain the Average Treatment Effect (ATE) (risk-difference)
\begin{align*}
E[Y(1)]  - E[Y(0)]
\end{align*}
...
Alternatively, we could get the estimate of the treatment effect on the log-odds
scale:
```{r est-ate-or}
...
We refer to the `targeted` package [@r-targeted] for a general implementation
where the nuisance models can be based on machine learning models using
cross-fitting. The above estimation can be obtained with the  `targeted::cate` function in the following way
```{r est-cate}
#| eval: false
