...
\[
\pr(Y_{ij} = 1 \mid U_{i}, W_{ij})\Phi(\mu_{j} + \beta_{j} W_{ij} + U_{i}), U_{i}\sim\mathcal{N}(0,\sigma_{u}^{2})
\]
...
sem <- lvm(y1 + y2 ~ 1 * u + w) |>
  latent(~u) |>
  ordinal(K=2, ~y1+y2)
estimate(sem, data = dw) |>
  estimate() |>
  summary() ## Robust standard errors
```
...
[\IC_{1}(Z_i; P)^\top,\ldots,\IC_{M}(Z_i; P)^\top]^{\top} + o_{p}(1) \\
&\Dto \mathcal{N}(0,\Sigma)
\end{align*}
by CLT, and
\(\frac{1}{n}\sum_{k=1}^n [\IC_{1}(Z_i; P)^\top,\ldots,\IC_{M}(Z_i; P)^\top]^{\top \otimes 2} \overset{P}{\longrightarrow}\Sigma\)
as \(n\to\infty\).
...
obtain from a GEE marginal model with working independence correlation structure
```{r}
...
TODO: method argument to estimate (Richardson vs complex)
