...
vignette: >
  %\VignetteIndexEntry{The Power of Influence}
...
- Quantiles, estimate.data.frame, estimate.array via kernel estimator
...
- multiple testing: pzmax, p.correct, closed testing procedure
...
set.seed(1)
y <- cbind(rnorm(1e5))
N <- 1e3 ## Number of aggregated groups, the number of observations in the new IF
ff <- foldr(NROW(y), N, 1)[[1]] ## Create the random group variable
ids <- rep(seq_len(length(ff)), unlist(lapply(ff, length)))
id <- ids[order(unlist(ff))]
...
As an example consider marginal models for the three binary response variables
  \(Y_1, Y_2, Y_3\) 
```{r}
g <- lapply(
  list(y1 ~ a, y2 ~ a, y3 ~ a),
...
The \(\bm{b}_0\) vector (default assumed to be zero) can be specied via the `null` argument
```{r}
...
```{r pairwise.diff}
pairwise.diff(3)
estimate(gg, pairwise.diff(3), use=c(2,4,6))
```
