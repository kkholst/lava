...
- Diagnostics Cohens Kappa, partial gamma example
...
Formally, let \(Z_1,\ldots,Z_n\) be iid \(k\)-dimensional stochastic variables, \(Z_i=(Y_{i},A_{i},W_{i})\sim P_0\), and \(\widehat{\theta}\) a
...
\frac{1}{\sqrt{n}}\sum_{i=1}^n \IC(Z_i; \theta) + o_p(1), 
\end{align*} 
where the
...
\begin{align*}
\log \frac{\pr(Y\leq j\mid x)}{1-\pr(Y\leq j\mid x)} = \alpha_{j} + \beta^{t}x, \quad j=1,\ldots,J
\end{align*}
```{r ordreg}
...
\[
\pr(Y_{ij} = 1 \mid U_{i}, W_{ij})=\Phi(\mu_{j} + \beta_{j} W_{ij} + U_{i}), U_{i}\sim\mathcal{N}(0,\sigma_{u}^{2}), j=1,2
\]
...
\begin{align*}
\IC(x; P) = \tau - \one(x\leq \beta)f(\beta)^{-1} 
\end{align*}

where \(f\) is the density function of \(X\).

To calculate the variance estimate an estimate of \(f\) is needed which can be
...
\underbrace{[\IC_{1}(Z_i; P)^\top,\ldots,\IC_{M}(Z_i; P)^\top]^{\top}}_{\overline{\IC}(Z_i; P)} + o_{p}(1) \\
&\Dto \mathcal{N}(0,\Sigma)
\end{align*}
by CLT, and
\(\mathbb{P}_{n}\overline{\IC}(Z_i; P)^{\otimes 2} \overset{P}{\longrightarrow}\Sigma\)
as \(n\to\infty\).
...
\frac{1}{\sqrt{N}}
\sum_{i=1}^{n} \sum_{k=1}^{N_{i}} IC(Z_{ik}; P) + o_{p}(1).
\]
It then follows that 
\[
\sqrt{n}(\widehat{\theta}-\theta) = 
\frac{1}{\sqrt{n}}
\sum_{i=1}^{n} \widetilde{\IC}(Z_{i}; P) + o_{p}(1)
\]
with \(\widetilde{\IC}(Z_{i}; P)
= \sum_{k=1}^{N_{i}} \frac{n}{N}IC(Z_{ik}; P)\), \(i=1,\ldots,n\) which are iid
...
independent is not consistent. Instead we can calcualte the cluster robust
...
\frac{1}{\sqrt{n}}\sum_{i=1}^n \nabla\phi(\theta)\IC(Z_i; \theta) + o_p(1), 
\end{align*} 
...
estimate(g1, exp)         
```
The gradient can be supplied as the attributes `grad`  and otherwise numerical differentiation is applied. 
...
is \(\IC(X1,X2; P) = (X_{1}-\E X_{1}, X_{2}-\E X_{2},
...
\begin{align*}
\IC_{x_1, x_2}(X_1, X_2; P) = (X_1 - \E X_1)(X_2 - \E X_2) - \cov(X_1,X_2)
\end{align*}
...
\(z = \arctanh(\widehat{\rho}) = \frac{1}{2}\log\left(\frac{1+\widehat{\rho}}{1-\widehat{\rho}}\right)\)
<!-- This estimate has an approximate -->
...
stabilizing transform does not have any impact.

## TODO Example: survival probabilities


## TODO Linear contrasts and hypothesis testing
