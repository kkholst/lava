...
```{r glmmargmis}
dd <- dw; rownames(dd) <- NULL
g3 <- glm(y3 ~ a, family=binomial, data=dw)
g3 <- glm(y3 ~ a, family=binomial, data=dw)
e2 <- estimate(g2, id = dw$id, keep = "a")
e3 <- estimate(g3, id = with(dw, id[!is.na(y3)]), keep="a")
e <- merge(e1, e3)
```
(or the overloaded `+` operator)

## Clustered data (non-iid)
