...
$$X_{j} = u_{1} + \epsilon_{j}^{x}, \quad j=1,2,3$$
$$Y_{j} = u_{2} + \epsilon_{j}^{y}, \quad j=1,2,3$$
and with a structural model given by
$$u_{2} = f(u_{1}) + Z + \zeta_{2}$$
$$u_{1} = Z + \zeta_{1}$$
...
term (`u2~u1_2`). For comparison we can also estimate the full MLE
of the linear model:
...
$$
\mathbb{E}_{\widehat{\theta}_{2}}(u_{2} \mid u_{1}, Z=0),
$$
...
ff <- lapply(
  list(linear = m2a, quadratic = m2b, spline4 = m2c, spline6 = m2d),
...
cross-validation (also for choosing the mixture distribution via the `nmix`
argument, see the section below). For example,

```{r  twostageCV, cache=TRUE, eval=fullVignette }
set.seed(1)
selmod <- twostageCV(m1, m2,
  data = d, df = 2:4, nmix = 1:2,
...
degrees of freedom varying from 1-3 (the linear model is
...
between the latent variable $u_{1}$ and a dichotomized version of
the covariate $z$
...
model. Thus, only the intercept of $u_{1}$ is allowed to vary
...
confband(pm[, 1],
  lower = pm[, 4], upper = pm[, 5], polygon = TRUE,
