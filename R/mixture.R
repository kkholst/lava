...
mixture <- function(
  ...
  if ("iter.max" %in% base::names(optim)) {
    optim$maxiter <- optim$iter.max
  }
  ...
  varpos <- c()
  for (i in 1:k) {
    vpos <- sapply(mg$parlist[[i]][variances(mg$lvm[[i]])], function(y) {
      as.numeric(substr(y, 2, nchar(y)))
    })
    offpos <- sapply(mg$parlist[[i]][offdiags(mg$lvm[[i]])], function(y) {
      as.numeric(substr(y, 2, nchar(y)))
    })
  ...
  if (!any(constrained)) {
    optim$constrain <- FALSE
  }

  mymodel <- list(multigroup = mg, k = k, data = data, parpos = ParPos)
  class(mymodel) <- "lvm.mixture"
  ...
  PosteriorProb <- function(pp, priorprob, constrain = FALSE) {
    if (!is.list(pp)) {
      ...
      if (missing(priorprob)) {
        priorprob <- pp[seq(Npar + 1, length(pp))]
      }
  ...
  negLogLik <- function(p) {
    ...
    K <- length(myp)
    prob <- p[seq(Npar + 1, Npar + K - 1)]
    prob <- c(prob, 1 - sum(prob))
    logff <- sapply(seq_along(myp), function(j) {
      (logLik(
        mg$lvm[[j]],
        p = myp[[j]],
        data = data,
  ...
  if (!is.null(em.control$trace)) {
    em.control$trace <- em.control$trace > 0
  }
  ...
  val <- c(
    val,
    list(
      member = apply(val$gamma, 1, which.max),
      k = k,
      data = data,
      parpos = ParPos,
      multigroup = mg,
...
score.lvm.mixture <- function(
  ...
  logff <- sapply(seq(x$k), function(j) {
    (logLik(
      x$multigroup$lvm[[j]],
      p = myp[[j]],
      data = x$data,
  ...
  scoref <- lapply(
    score(x$multigroup, p = p, indiv = TRUE, model = model),
    function(x) {
      x[which(is.na(x))] <- 0
      x
    }
...
##' @export
information.lvm.mixture <- function(
  x,
  p = coef(x, full = TRUE),
  ...,
  type = "observed"
...
##' @export
logLik.lvm.mixture <- function(
  object,
  p = coef(object, full = TRUE),
  prob,
  model = "normal",
  ...
) {
  ...
  logff <- sapply(seq(object$k), function(j) {
    (logLik(
      object$multigroup$lvm[[j]],
      p = myp[[j]],
      data = object$data,
      indiv = TRUE,
      model = model
