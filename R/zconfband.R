...
##' forrestplot(x,sep=c(2,15),col=signif+1)
...
##' @export
forrestplot <- function(x,lower,upper,vline=0,labels,text=TRUE,axes=TRUE,cex=1,pch=15,xlab="",ylab="",sep,delta,xlim,ylim,mar=c(4,8,1,1),...) {
    if (is.matrix(x)) {
        lower <- x[,2]; upper <- x[,3]
        if (ncol(x)>3) cex <- x[,4]
        x <- x[,1]
    ...
    K <- length(x)
    op <- par(mar=mar) ## bottom,left,top,right
    on.exit(par(op))
    if (missing(ylim)) ylim <- c(1,K)
    if (missing(xlim)) {
        if (missing(delta)) delta <- max(upper-lower,na.rm=TRUE)*0.4
        xlim <- range(c(x,lower-delta,upper+delta),na.rm=TRUE)
    }
    ...
    confband(seq(K),lower,upper,x,pch=pch,cex=cex,vert=FALSE,delta=0,blank=FALSE,...)
    mtext(labels,2,at=seq(K),las=2)
    if (text)
        for (i in seq_len(K)) {
            st <- paste(formatC(x[i])," (",formatC(lower[i]),";",formatC(upper[i]),")",sep="")
            graphics::text(upper[i],i,st,xpd=TRUE,pos=4,cex=0.6)
        }
}
