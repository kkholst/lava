...
substArg <- function(x, env, ...) {
  ...
  if (is.null(myclass) || myclass == "name") {
    res <- unlist(sapply(as.character(a), function(z) {
...
`procdata.lvm` <-
  function(
    ...
    categorical = FALSE,
    na.method = ifelse(
      any(is.na(data[, intersect(colnames(data), manifest(x))])),
      "complete.obs",
    ...
    if (is.data.frame(data) | is.matrix(data)) {
      ...
      if (na.method == "complete.obs" && !missing) {
        ...
        if (is.null(S) && n > 2) {
          S <- (n - 1) / n * cov(mydata0)
        } ## MLE variance matrix of observed variables
        rm(mydata0)
      }
...
`symmetrize` <-
  function(M, upper = TRUE) {
    ...
    if (!is.matrix(M) | ncol(M) != nrow(M)) {
      stop("Only implemented for square matrices.")
    }
    if (upper) {
...
getoutcome <- function(formula, sep, ...) {
  ...
  if (!missing(sep) && length(aa$term.labels) > 0) {
    attributes(res)$x <- lapply(
      strsplit(aa$term.labels, "\\|")[[1]],
