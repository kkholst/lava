...
predict.lvm <- function(
  ...
  if (!quick && !all(exogenous(object) %in% colnames(data))) {
    stop("data.frame should contain exogenous variables")
  }
  m <- moments(object, p, data = data, ...)
  if (quick) {
    ## Only conditional moments given covariates
    ii <- index(object)
    P.x <- m$P
    P.x[ii$exo.idx, ii$exo.idx] <- 0
    Cy.x <- (m$IAi %*% tcrossprod(P.x, m$IAi))[
      ii$endo.idx,
      ii$endo.idx,
      drop = FALSE
    ]
  ...
  if (path) {
    ...
    if (length(Y) == 0) {
      stop(
        "New data set should only contain exogenous variables and a true subset of the endogenous variables for 'path' prediction."
      )
    }
    A <- t(m$A)
    A[, idx] <- 0 ## i.e., A <- A%*%J
  ...
  if (length(X) > 0) {
    ...
    mu.x <- apply(xs, 1, FUN = function(i) {
      res <- rep(0, length(mu.0))
      res[X.idx.all] <- i
      res
    })
    xi.x <- (IAi %*% (mu.0 + mu.x))
  ...
  if (!is.null(x)) {
    ...
    if (length(x) > 0) {
      E.x <- xi.x[y, , drop = FALSE] +
        C.x[y, x] %*% solve(C.x[x, x]) %*% ry[x, , drop = FALSE]
    } else {
      E.x <- xi.x[y, , drop = FALSE]
    }
    if (residual) {
      Vhat <- matrix(0, nrow(data), length(vars(object)))
      colnames(Vhat) <- vars(object)
  ...
  epsilonhat <- (t(IA %*% t(Vhat) - m$v))[,
    c(endogenous(object), latent(object)),
    drop = FALSE
  ]
  if (residual) {
    return(epsilonhat)
  ...
  mydata <- matrix(0, ncol = ncol(A), nrow = nrow(data))
  colnames(mydata) <- vars(object)
...
predictlvm <- function(
  ...
  J <- matrix(seq(k^2), k)
  if (length(idxX) == 0) {
    ## Return conditional mean and variance given covariates
