twostagelvm <- function(
  ...
  if (!inherits(model2, c("lvm"))) {
    stop("Expected lava object ('lvm',...)")
  }
  if (!is.null(formula)) {
  ...
  if (model.object) {
    ...
    return(structure(
      list(model = model, nonlinear = nonlin, call = cl),
      class = "twostage.lvm"
    ))
  }
...
twostage.lvmfit <- function(
  ...
  if (std.err) {
    if (is.null(id1)) {
      id1 <- seq(nrow(model.frame(object)))
    }
    if (is.null(id2)) {
      id2 <- seq(nrow(model.frame(model2)))
    }
    model1 <- object
  ...
  structure(
    res,
    class = c("twostage.lvmfit", "measurement.error", "lvmfit", "estimate")
  )
}
...
estimate.twostage.lvm <- function(x, data, ...) {
  ...
  if (!inherits(m1, "lvmfit")) {
    ...
    args <- args[intersect(
      names(as.list(base::args(estimate.lvm))),
      names(args)
    )]
  ...
  twostage(
    ...
    model2 = m2,
    data = data,
    predict.fun = nl[[1]]$pred,
    ...
  )
}

##' @export
twostage.twostage.lvm <- function(object, ...) {
  estimate.twostage.lvm(object, ...)
}
...
##' @export
predict.twostage.lvmfit <- function(
  object,
  newdata,
  ...
  if (missing(newdata)) {
    stop("provide data for prediction")
  }
  nl <- nonlinear(object)
...
twostageCV <- function(
  ...
  if (length(F) == 0) {
    stop(
      "Specify at least one nonlinear association in 'model2' (using the 'nonlinear' method)"
    )
  }
  ...
  startf <- function(n) {
    u0 <- seq(knots.boundary[1], knots.boundary[2], length.out = n)
    names(u0) <- paste0("p", seq_along(u0))
    c(pint, u0, p0[-intpos])
  }
  control1$start <- NULL
  ee <- list(e1a)
  ...
  fit <- function(k, ...) {
    if (messages > 0) {
      cat("Fitting mixture model with", k, "components\n")
    }
    mixture(
      model1,
      k = k,
  ...
  for (i in df) {
    ...
    MM <- c(MM, list(nonlinear(model2, form, type = "spline", knots = knots)))
  }
    ...
    f0 <- function(data) {
      list(
        e0 = estimate(
          model1,
          data = data,
          control = c(control1, list(start = coef(e1)))
        )
      )
    }
  } else {
  ...
  ff <- lapply(MM, function(m) {
    function(data, e0, ...) twostage(e0, m, data = data, std.derr = FALSE)
  })
