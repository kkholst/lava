...
mat.lvm <- function(x, ii = index(x), ...) {
  ...
  if (length(pars.off.diag) > 0 & ncol(P) > 1) {
    for (i in seq_len(ncol(P1) - 1)) {
      for (j in seq(i + 1, nrow(P1))) {
        if (ii$P1[j, i] != 0) {
          counter <- counter + 1
          pos <- c(j + (i - 1) * ncol(P1), i + (j - 1) * ncol(P1))
          P[j, i] <- P[i, j] <- pars.off.diag[counter]
          idxP <- c(idxP, pos)
          pidxP <- c(pidxP, P[j, i], P[i, j])
        }
      }
    }
  }
  ...
  ee <- cbind(idxE, pidxE)
  rownames(ee) <- names(x$expar)[ee[, 1]]
  ...
  meanl <- idxM
  if (!is.null(meanl)) {
    meanl <- cbind(meanl, pidxM)
  }
  regl <- idxA
  if (!is.null(regl)) {
    regl <- cbind(regl, pidxA)
  }
  covl <- idxP
  if (!is.null(covl)) {
    covl <- cbind(covl, pidxP)
  }
...
matrices.lvm <- function(
  ...
  if (length(constrain.par) > 0 && is.numeric(c(pars, meanpar))) {
    ...
    for (p in constrain.par) {
      ...
      cpar <- myc(val)
      constrainpar <- c(constrainpar, list(cpar))
  ...
  return(list(
    ...
    e = e,
    parval = parval,
    constrain.idx = ii$constrain.idx,
    constrainpar = constrainpar
  ))
}
...
matrices.lvm <- function(
  ...
  if (length(constrain.par) > 0 && is.numeric(c(pars, meanpar, e))) {
    ...
    for (p in constrain.par) {
      ...
      val <- rbind(unlist(c(parval, constrainpar, x$mean, e)[
        attributes(myc)$args
      ]))
      cpar <- myc(val)
      constrainpar <- c(constrainpar, list(cpar))
