...
`gaussian_objective.lvm` <-
  function(x, p, data, S, mu, n, ...) {
    ...
    if (!lava.options()$allow.negative.variance && any(diag(mp$P) < 0)) {
      return(NaN)
    }
...
gaussian_score.lvm <- function(
  ...
  if (!is.null(data)) {
    if ((nrow(data) < 2 | !is.null(weights)) | indiv) {
      ...
      for (i in seq_len(NROW(data))) {
        ...
        if (!is.null(weights)) {
          W <- W0
          diag(W)[widx] <- as.numeric(weights[i, ])
          score[i, ] <-
            as.numeric(
              crossprod(u, iC %*% W) %*%
                D$dxi +
  ...
  if (lava.options()$devel) {
    ...
    Grad[-imean] <- n /
      2 *
      crossprod(D$dS[, -imean], as.vector(iC %*% T %*% iC) - vec.iC)
  } else {
  ...
  if (!is.null(mu) & !is.null(xi)) {
    if (!(lava.options()$devel)) {
      ...
      Grad[with(index(x)$parBelongsTo, c(mean, reg))] <- Grad[with(
        index(x)$parBelongsTo,
        c(mean, reg)
      )] -
        (n / 2 * crossprod(D$dT, vec.iC))
    }
  }
  res <- as.numeric(Grad)
  return(rbind(res))
