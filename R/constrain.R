...
Range.lvm <- function(a = 0, b = 1) {
  ...
  f <- function(x) {
    ...
    attributes(res)$grad <- function(x) {
      exp(x) * (b - a - a * b * exp(x)) / (1 + exp(x))^2
    }
    res
  }
  return(f)
...
"constrain<-.default" <- function(x, par, args, endogenous = TRUE, ..., value) {
  ...
  for (i in args) {
    if (!(i %in% c(parlabels(Model(x)), vars(Model(x)), names(constrain(x))))) {
...
constraints <- function(
  ...
  if (class(object)[1] == "multigroupfit") {
    ...
    return(
      attributes(CoefMat.multigroupfit(
        object,
        data = data,
        vcov = vcov,
        ...
  ...
  if (NROW(data) > 1 & !missing(idx)) {
    res <- t(apply(
      ...
      1,
      function(x) {
        constraints(object, data = x, p = p, vcov = vcov, level = level)[idx, ]
      },
      ...
    ))
    return(res)
  ...
  if (length(index(object)$constrain.par) < 1) {
    return(NULL)
  }
  ...
  if (is.null(parpos)) {
    parpos <- with(
      index(object),
      matrices2(Model(object), seq_len(npar + npar.mean + npar.ex))
    )
    ...
    if (length(vals0) == 0) {
      ...
      if (!is.null(attributes(fval)$grad)) {
        ...
        if (fmat) {
          ...
          Gr <- cbind(as.numeric(numDeriv::jacobian(
            myc0,
            rbind(unlist(vals0))
          )))
        }
