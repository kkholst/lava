### NOTE # [OBmodification] line 560 577 590 to adding ...
...
`estimate.lvm` <-
  function(x, data=parent.frame(),
    ...
    if (length(xfix)>0 | (length(xconstrain)>0 & XconstrStdOpt | !lava.options()$test)) { ## Yes
      stop("[OB modif]: random slope no ready \n")
...
#       ##################################################
      ...
      myObj <- function(pp, ...) { #[OB-modif]
        ...
        do.call(ObjectiveFun, list(x=x0, p=pp, data=data, S=S0, mu=mu0, n=n, weight=weight
                                   ,weight2=weight2, offset=offset, ...  #[OB-modif]
        ))
      }
      
      myGrad <- function(pp, ...) {  #[OB-modif]
        # print("Grad begin")
        if (optim$constrain)
          pp[constrained] <- exp(pp[constrained])
        ##  offset <- MkOffset(pp)
    ...
    myHess <- function(pp, ...) {  #[OB-modif]
      # print("Hessian begin")
      p0 <- pp
      if (optim$constrain)
        pp[constrained] <- exp(pp[constrained])
      I0 <- myInfo(pp, ...)  #[OB-modif]
      
      attributes(I0)$grad <- NULL
      D <- attributes(I0)$grad
      if (is.null(D)) {
        D <- myGrad(p0, ...)  #[OB-modif]
    ...
    if (!is.null(optim$method)) {
      ...
      if (XconstrStdOpt & !is.null(myGrad))
        opt$gradient <- as.vector(myGrad(opt$par, ...)) #[OB-modif]
      else {
...
nlminb2 <- function(start,objective,gradient,hessian,...) {
  ...
  nlminbcontrols <- c("eval.max", "iter.max", "trace", "abs.tol", 
                      "rel.tol", "x.tol", "step.min")
  dots <- list(...)
  control <- list(...)$control
  control <- control[names(control) %in% nlminbcontrols]
  dots$control <- control
  dots$lower[length(dots$lower)] <- 0.000001
  if (length(dots$trace) > 0 && dots$trace > 0) 
    cat("\n")
  mypar <- c(list(start = start, objective = objective, gradient = gradient, 
...
#   mypar$hessian(mypar$start, ...)
#   numDeriv:::hessian(mypar$objective, mypar$start)
#   
#   res0 <- do.call("nlminb", mypar)
#   res <- optimx(par = mypar$start, 
#          fn = mypar$objective, 
#          gr = mypar$gradient,
#          hess = NULL, # mypar$hessian,
#          #method = "L-BFGS-B",
#          lower = mypar$lower)
