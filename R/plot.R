...
`plot.lvm` <-
  function(
    ...
    if (lava.options()$debug) {
      ...
      if (attributes(g)$feedback) {
        dots$recipEdges <- c("combine")
      }
      if (is.null(dots$layoutType)) {
        dots$layoutType <- layout[1]
      }
      if (all(index(x)$A == 0)) {
      ...
      if (noplot) {
        return(suppressMessages(graph::updateGraph(g)))
      }
...
vis.lvm <- function(
  ...
  if (cor) {
    ...
    if (nrow(coredges) > 0) {
      if (!labels) {
        coredges <- coredges[, 1:2, drop = FALSE]
      }
  ...
  v <- visNetwork::visEdges(
    v,
    arrows = list(from = list(enabled = TRUE, scaleFactor = 0.5)),
...
beautify <- function(
  ...
  if (length(keep) > 0) {
    ...
    lab <- paste0(
      ...
      '"',
      "=",
      paste0("expression(", trim, "[scriptscriptstyle(", num, ")])"),
      collapse = ","
    )
  ...
  for (i in ila) {
    for (j in which(x$M[i, ] == 1)) {
      ...
      if (is.null(try(eval(elab), silent = TRUE))) {
        elab <- ""
      }
      edgelabels(
        x,
