...
##' @export
information.lvm <- function(
  x,
  p,
  n,
  type = ifelse(
    model == "gaussian",
    c("E", "hessian", "varS", "outer", "sandwich", "robust", "num"),
    "outer"
  ),
  data,
  weights = NULL,
  ...
  if (
    type[1] %in%
      c("num", "hessian", "obs", "observed") |
      (type[1] %in% c("E", "hessian") & model != "gaussian")
  ) {
    myf <- function(p0) {
      score(
        x,
  ...
  if (n > 1) {
    ...
    if (length(xfix) > 0 | length(xconstrain) > 0) {
      ## Random slopes!
      x0 <- x
      if (length(xfix) > 0) {
        nrow <- length(vars(x))
      ...
      myfun <- function(ii) {
        if (length(xfix) > 0) {
          for (i in seq_along(myfix$var)) {
            for (j in seq_along(myfix$col[[i]])) {
              index(x0)$A[cbind(myfix$row[[i]], myfix$col[[i]])] <- data[
                ii,
                myfix$var[[i]]
              ]
            }
      ...
      val <- apply(
        array(unlist(L), dim = c(length(p0), length(p0), nrow(data))),
        c(1, 2),
        sum
      )
      if (inverse) {
  ...
  if (!is.null(weights) && is.matrix(weights)) {
    L <- lapply(seq_len(nrow(weights)), function(y) {
      information(x, p = p, n = 1, type = type, weights = weights[y, ])
    })
    val <- apply(
      array(unlist(L), dim = c(length(p), length(p), nrow(weights))),
      c(1, 2),
      sum
    )
    if (inverse) {
  ...
  if (is.null(weights)) {
    ...
    if (lava.options()$devel) {
      ...
      information_Sigma[-imean, -imean] <- n /
        2 *
        t(D$dS[, -imean]) %*% kronprod(iC, iC, D$dS[, -imean])
    } else {
  ...
  if (!(lava.options()$devel)) {
    ...
    information[mparidx, mparidx] <- information[mparidx, mparidx] +
      information_mu
  }
...
##' @export
information.lvm.missing <- function(
  x,
  p = coef(x),
...
##' @export
information.multigroupfit <- function(
  x,
  p = pars(x),
  weights = Weights(x),
  estimator = x$estimator,
...
##' @export
information.multigroup <- function(
  x,
  data = x$data,
  weights = NULL,
  p,
  ...
  if (!indiv) {
    for (i in seq_len(x$ngroup)) {
      I[parord[[i]], parord[[i]]] <- I[parord[[i]], parord[[i]]] +
        information(
          x$lvm[[i]],
          p = pp[[i]],
          data = data[[i]],
