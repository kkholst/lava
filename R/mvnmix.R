...
mvnmix <- function(
  ...
  for (ii in seq(n.start)) {
    ...
    while ((i < steps) & (E >= tol)) {
      ...
      lphis <- c()
      for (j in 1:k) {
        C <- matrix(Sigmas[j, ], ncol = D)
        diag(C) <- diag(C) + lambda ## Assure C is not singular
  ...
  if (is.null(myvars)) {
    myvars <- colnames(data) <- paste("y", 1:NCOL(data), sep = "")
  }
  data <- as.data.frame(data)
  m <- lvm(myvars, silent = TRUE)
  m <- covariance(m, myvars, pairwise = TRUE)
  models <- datas <- c()
  for (i in 1:k) {
  ...
  for (i in 1:k) {
    parpos <- c(
      ...
      list(c(
        seq_len(D) + (i - 1) * D,
        k * D + seq_len(npar1) + (i - 1) * (npar1)
      ))
    )
  }

  theta <- c(
    unlist(lapply(getMeanVar(res), function(x) x$mean)),
    unlist(lapply(getMeanVar(res), function(x) {
      c(diag(x$var), unlist(x$var[upper.tri(x$var)]))
    }))
  )
...
print.mvn.mixture <- function(x, ...) {
  ...
  for (i in 1:x$k) {
    ...
    cat(
      "\tcenter = \n ",
      space,
      paste(formatC(par$mu[i, ]), collapse = " "),
      sep = ""
    )
    cat("\n\tvariance = \t")
    V <- matrix(formatC(par$Sigma[i, ], flag = " "), ncol = x$D)
    colnames(V) <- rep("", x$D)
    rownames(V) <- rep(space, x$D)
    print(V, quote = FALSE)
    cat("\n")
  }
  invisible(par)
...
sim.mvn.mixture <- function(x, n, ...) {
  ...
  res <- c()
  for (i in seq(K)) {
    res <- rbind(res, mets::rmvn(ng[i], pars[[i]]$mean, pars[[i]]$var))
  }
  return(res)
}
