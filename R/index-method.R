...
##' @seealso [modelPar()]
##' @export
"index.lvm" <- function(x, ...) {
  x$index
}

##' @export
"index.lvmfit" <- function(x, ...) {
  index(Model(x))
}
...
##' @aliases index<-.lvmfit index<-.lvm
##' @export
"index<-.lvm" <- function(x, ..., value) {
  x$index <- value
  return(x)
}

##' @export
"index<-.lvmfit" <- function(x, ..., value) {
  Model(x)$index <- value
  return(x)
}
...
`reindex` <-
  function(
    ...
    ## Extract indices of parameters from model
    x$parpos <- NULL
    M <- x$M
    ...
    M0 <- M
    M0[fix.idx] <- 0 ## Matrix of indicators of free regression-parameters (removing fixed parameters)
    ...
    P0 <- P
    P0[covfix.idx] <- 0 ## Matrix of indicators of free covariance-parameters (removing fixed parameters)
    if (length(exo.idx) > 0) {
      P0[exo.idx, exo.idx] <- 0
    } ## 6/1-2011
    ...
    } ## Selection matrix (selecting observed variables)
    {
      ...
      if (length(c(eta.idx, exo.idx)) > 0) {
        Jy[c(eta.idx, exo.idx), c(eta.idx, exo.idx)] <- 0
        Jy <- Jy[-c(eta.idx, exo.idx), , drop = FALSE]
      }
    ...
    res <- list(
      ...
      exo.idx = exo.idx,
      eta.idx = eta.idx,
      exo.obsidx = exo.obsidx,
      endo.obsidx = endo.obsidx,
      obs.idx = obs.idx,
    ...
    if (standard) {
      ...
      res <- c(
        ...
        list(
          parname.all = parname,
          parname = setdiff(parname, constrain.par),
      ...
      if (length(parname.all.reg.idx) > 0) {
        names(parname.all.reg.idx) <- names(
          parname.all.reg.tidx
        ) <- res$parname.all
      }
