...
multigroup <- function(
  ...
  nm <- length(models)
  if (nm != length(datasets)) {
    stop("Supply dataset for each model")
  }
  if (nm < 2) {
    stop("Two or more groups neeeded")
  }
  ...
  if (missing) {
    ...
    nmis <- c()
    for (i in seq_len(nm)) {
      myvars <- unlist(intersect(
        colnames(datasets[[i]]),
        c(vars(models[[i]]), xfix[[i]], keep)
      ))
  ...
  for (i in seq_len(nm)) {
    if (inherits(datasets[[i]], c("data.frame", "matrix"))) {
      ...
      if (any(is.na(datasets[[i]][, myvars]))) {
        if (!warned) {
          warning(paste0(
            "Missing data encountered. Going for complete-case analysis"
          ))
        }
        warned <- TRUE
  ...
  for (i in seq_len(nm)) {
    ...
    P <- index(mymodel)$P1
    P[P == 0] <- NA
    P[!is.na(P) & !is.na(mymodel$covpar)] <- mymodel$covpar[
      !is.na(P) & !is.na(mymodel$covpar)
    ]

    A <- index(mymodel)$M1
    A[A == 0] <- NA
    A[!is.na(A) & !is.na(mymodel$par)] <- mymodel$par[
      !is.na(A) & !is.na(mymodel$par)
    ]
  ...
  for (i in seq_len(length(myparPos))) {
    ...
    if (length(p1) > 0) {
      coefs <- c(
        coefs,
        paste(
          coef(lvms[[i]], fix = FALSE, mean = FALSE)[pp0[[i]]],
          i,
          sep = "@"
        )
      )
...
checkmultigroup <- function(x) {
  ...
  for (i in seq_len(x$ngroup)) {
    if (nrow(x$data[[i]]) < 2) {
      warning(
        "With only one observation in the group, all parameters should be inherited from another a group!"
      )
    }
  }
}
