...
rsq <- function(x, stderr = FALSE) {
  if (stderr) {
    ...
    ic.mod <- IC(x)
    coef0 <- c(attributes(ic.v)$coef[vpar], coef(x)[vpar])
  ...
  res <- coef(x, 9, std = "yx")
  idx <- with(attributes(res), which(type == "variance" & (var == from)))
  ...
  if (object$estimator == "gaussian" & logLik & !missing) {
    if (inherits(object, c("multigroupfit", "multigroup"))) {
      ...
      for (i in seq_len(Model(object)$ngroup)) {
        l0 <- logLik(
          Model(Model(object))[[i]],
          data = model.frame(object)[[i]],
          type = "sat"
        )
...
gof.lvmfit <- function(
  ...
  chisq = FALSE,
  level = 0.90,
  rmsea.threshold = 0.05,
  all = FALSE,
  ...
) {
  n <- object$data$n
  if (inherits(object, "multigroupfit")) {
    n <- sum(unlist(lapply(object$model$data, nrow)))
  }
  ...
  myAIC <- -2 * (loglik - df)
  attributes(myAIC) <- NULL
  myBIC <- -2 * loglik + df * log(nobs)
  attributes(myBIC) <- NULL

  xconstrain <- intersect(
    unlist(lapply(constrain(object), function(z) attributes(z)$args)),
    manifest(object)
  )
  ...
  if (
    ((object$estimator == "gaussian" & !inherits(object, "lvm.missing")) |
      chisq) &
      length(xconstrain) == 0
  ) {
    res <- list(
    ...
    if (all) {
      ...
      if (is.null(S)) {
        S <- cov(model.frame(object))
      }
      if (is.null(mu)) {
        mu <- colMeans(model.frame(object))
      }
      L <- diag(S)^0.5
      ...
      res <- c(
        res,
        list(CFI = CFI, NFI = NFI, TLI = TLI, C = C, S = S, SRMR = SRMR)
      )
    ...
    if (rnkV == ncol(vcov(object)) && (!is.null(minSV) && minSV > 1e-12)) {
      rmseafun <- function(...) {
        epsilon <- function(lambda) {
          sapply(lambda, function(x) {
            ifelse(x > 0 & qdf > 0, sqrt(x / (qdf * (n))), 0)
          })
        } ## n-1,n vs. n-df
        ...
        res <- list(
          ...
          RMSEA = RMSEA,
          level = level,
          rmsea.threshold = rmsea.threshold,
          pval.rmsea = pval
        )
        return(res)
      }
...
print.gof.lvmfit <- function(x, optim = TRUE, ...) {
  ...
  if (is.null(x$fit)) {
    with(x, cat(" Log-Likelihood =", logLik, "\n"))
  }
  with(x, cat(" BIC =", BIC, "\n", "AIC =", AIC, "\n"))
  if (!is.null(x$fit)) {
    with(
