...
sim.lvm <- function(
  ...
  loadconfig <- !is.null(x$sim.env) &&
    !setup &&
    (length(list(...)) == 0 && length(p) == 0)
  Yfix <- NULL
  ...
  if (loadconfig) {
    ...
    mu <- unlist(lapply(x$mean, function(l) {
      ifelse(is.na(l) | is.character(l), 0, l)
    }))
    xf <- intersect(unique(parlabels(x)), xx)
    xfix <- c(randomslope(x), xf)
    if (length(xfix) > 0) {
      normal <- FALSE
    }
    ## Match parameter names
    if (
      (!is.null(names(p)) && all(!is.na(names(p)))) ||
        length(p) != (index(x)$npar + index(x)$npar.mean + index(x)$npar.ex) |
        is.null(names(p))
    ) {
      nullp <- is.null(p)
      p0 <- p
  ...
  if (!setup) {
    ...
    if (length(ii.mvn) > 0) {
      ...
      if (PPdiag) {
        for (i in ii.mvn) {
          E[, i] <- rnorm(n, sd = PP[i, i])
        }
      } else {
        E[, ii.mvn] <- matrix(
          rnorm(length(ii.mvn) * n),
          ncol = length(ii.mvn)
        ) %*%
          PP[ii.mvn, ii.mvn, drop = FALSE]
      }
    }
    ...
    if (length(mdistnam) > 0) {
      ...
      for (i in seq_along(fun)) {
        ...
        ii <- match(mv, vv)
        E[, ii] <- distribution(x, multivariate = TRUE)$fun[[i]](
          n,
          p = p,
          object = x
        ) # ,...)
  ...
  if (!loadconfig) {
    ...
    if (length(x$attributes$multitransform) > 0) {
      ...
      for (i in (seq_along(x$attributes$multitransform))) {
        multitrans.idx <- c(
          multitrans.idx,
          rep(i, length(x$attributes$multitransform[[i]]$y))
        )
      }
    }
    ...
    if (is.null(X) || NCOL(X) < length(xx)) {
      if (!is.null(xx) && length(xx) > 0) {
        for (i in seq_along(xx)) {
          ...
          if (is.list(dist.x) && is.function(dist.x[[1]])) {
            dist.x <- dist.x[[1]]
          }
          if (is.list(dist.x)) {
            dist.x <- dist.x[[1]]
          ...
          if (is.function(dist.x)) {
            res[, X.idx[i]] <- dist.x(
              n = n,
              mu = mu.x,
              var = P[X.idx[i], X.idx[i]]
            )
          } else {
            if (is.null(dist.x) || any(is.na(dist.x))) {} else {
              if (length(dist.x) != n) {
                stop(
                  "'",
                  vv[X.idx[i]],
  ...
  if (
    normal |
      (is.null(distribution(x)) &
        is.null(functional(x)) &
        is.null(constrain(x)))
  ) {
    if (cond) {
      ## Simulate from conditional distribution of Y given X
      mypar <- pars(x, A, P, mu)
      ...
      res <- cbind(yy, res[, xx])
      colnames(res) <- c(colnames(Vy.x), xx)
      return(res)
    }
    ...
    if (!loadconfig) {
      xc <- index(x)$vars
      xconstrain.idx <- unlist(lapply(
        lapply(constrain(x), function(z) attributes(z)$args),
        function(z) length(intersect(z, xc)) > 0
      ))
      xconstrain <- intersect(
        unlist(lapply(constrain(x), function(z) attributes(z)$args)),
        xc
      )
    ...
    if (setup) {
      ...
      for (v in v.env) {
        if (!is.null(get(v))) sim.env[[v]] <- get(v)
      }
      x$sim.env <- sim.env
      return(x)
    }
    ...
    if (length(xconstrain) > 0) {
      for (i in which(xconstrain.idx)) {
        if (!(names(xconstrain.idx[i]) %in% nn)) {
          ...
          if (inherits(val, "try-error") || NROW(val) < n) {
            apply(D, 1, ff)
          }
          res <- cbind(res, val)
          colnames(res)[ncol(res)] <- names(xconstrain.idx)[i]
        }
      }
    }
    ...
    for (i in seq_len(length(constrain(x)))) {
      ...
      if (length(xx) > 0) {
        ...
        y <- names(which(unlist(lapply(intercept(x), function(x) {
          x == parname
        }))))
    ...
    if (length(vartrans) > 0) {
      ...
      if (length(parvalsnam) > 0) {
        ...
        res <- cbind(res, cbind(rep(1, nrow(res))) %x% rbind(Parvals))
    ...
    while (length(simuled) < length(nn)) {
      ...
      if (
        !is.null(leftoversPrev) && length(leftoversPrev) == length(leftovers)
      ) {
        if (itercount > 0) {
      ...
      for (i in leftovers) {
        ...
        if (i %in% vartrans) {
          ...
          if (length(ipos) == 0 || all(xconstrain[[ipos]]$exo %in% simuled)) {
            ...
            if (all(c(relations, simvars, dist.xx) %in% simuled)) {
              ## Only depending on already simulated variables
              ...
              if (length(ipos) > 0) {
                ...
                X <- with(
                  ...
                  if (length(pp) > 0) {
                    cbind(
                      rbind(pp) %x% cbind(rep(1, nrow(res))),
                      res[, exo, drop = FALSE]
                    )
                  } else {
                    res[, exo, drop = FALSE]
                ...
                yy <- try(
                  with(xconstrain[[ipos]], func(X[, myidx, drop = FALSE])),
                  silent = TRUE
                )
                if (NROW(yy) != NROW(res)) {
                  ## apply row-wise
              ...
              if (!is.function(dist.i)) {
                ...
                if (length(simvars) > 0) {
                  ## Depends on mu and also on other variables (e.g. time-depending effect)
                  if (length(mu.i) == 1) {
                    mu.i <- rep(mu.i, n)
                  }
  ...
  if (!latent && length(latent(x)) > 0) {
    return(subset(res[, -which(colnames(res) %in% latent(x))]))
  }
  return(res)
}
