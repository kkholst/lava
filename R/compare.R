...
compare.default <- function(
  ...
  if (!missing(par) || (!missing(contrast) && is.character(contrast))) {
    if (!missing(contrast) && is.character(contrast)) {
      par <- contrast
    }
  ...
  if (!missing(contrast)) {
    ...
    pname <- names(p)
    B <- rbind(B)
    colnames(B) <- if (is.vector(contrast)) {
      names(contrast)
    } else {
      colnames(contrast)
    }
    ...
    if (!is.null(pname)) {
      ...
      for (i in seq_len(nrow(B))) {
        ...
        cnames <- c(
          cnames,
          paste0(sgn, Bval, paste0("[", pname[Bidx], "]"), collapse = "")
        )
  ...
  if (!missing(scoretest)) {
    ...
    if (inherits(scoretest, "formula")) {
      scoretest <- list(scoretest)
    }
    for (i in scoretest) {
    ...
    attributes(Q) <- NULL
    names(Q) <- "chisq"
    df <- length(p0) - length(coef(object))
    names(df) <- "df"
    pQ <- ifelse(df == 0, NA, pchisq(Q, df, lower.tail = FALSE))
    res <- list(
      data.name = as.character(scoretest),
      statistic = Q,
      parameter = df,
      p.value = pQ,
      method = "- Score test -"
  ...
  if (length(objects) < 2) {
    ...
    df <- attributes(L1)$df - attributes(L0)$df
    names(df) <- "df"
    Q <- abs(2 * (L0 - L1))
    attributes(Q) <- NULL
    names(Q) <- "chisq"
...
comparepair <- function(x1, x2) {
  ...
  df1 <- attributes(l1)$df
  df2 <- attributes(l2)$df
  if (is.null(df1)) {
  ...
  values <- c(l1, l2)
  names(values) <- c("log likelihood (model 1)", "log likelihood (model 2)")
