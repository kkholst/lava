ordreg_threshold <- function(theta) {
  v <- theta[1]
  if (length(theta) > 1) {
    v <- cumsum(c(v, exp(theta[seq(length(theta) - 1L) + 1L])))
  }
  return(v)
}
...
ordreg_ithreshold <- function(v) {
  theta <- v[1]
  if (length(v) > 1) {
    theta <- c(theta, log(-rev(diff(rev(v)))))
  }
  return(theta)
}
...
ordreg_dthreshold <- function(theta) {
  ...
  Da <- matrix(0, K, K - 1)
  Da[seq(K - 1), 1L] <- 1L
  for (i in seq_len(K - 2) + 1) {
    Da[seq(i, K - 1), i] <- exp(theta[i])
  }
  Da
}
...
ordreg <- function(
  ...
  assign(
    "threshold",
    function(theta, K) ordreg_threshold(theta[seq(K - 1)]),
    envir = up
  )
  assign(
    "dthreshold",
    function(theta, K) ordreg_dthreshold(theta[seq(K - 1)]),
    envir = up
  )
  ff <- function(theta) -ordreg_logL(theta, up)
  gg <- function(theta) -ordreg_score(theta, up)
  if (missing(start)) {
    start <- with(up, c(rep(-1, up$K - 1), rep(0, p)))
  }
...
summary.ordreg <- function(object, alpha = 0.95, ...) {
  ...
  res <- cbind(
    ...
    res[, 1] - res[, 2] * qq,
    res[, 1] + res[, 2] * qq,
    2 * (1 - pnorm(abs(res[, 1]) / res[, 2]))
  )
  colnames(res) <- c(
    "Estimate",
    "Std.Err",
    paste0(round(c(1 - pp, pp) * 1000) / 10, "%"),
    "P-value"
  )
...
ordreg_logL <- function(theta, env, indiv = FALSE, ...) {
  if (length(theta) != with(env, p + K - 1)) {
    stop("Wrong dimension")
  }
  env$theta <- theta
  if (env$p > 0) {
    beta <- with(env, theta[seq(p) + K - 1])
  }
...
ordreg_score <- function(theta, env, ...) {
  if (!identical(theta, env$theta)) {
    ordreg_logL(theta, env)
  }
...
##' @export
predict.ordreg <- function(
  object,
  p = coef(object),
  type = c("prob", "cumulative"),
  ...
) {
  env <- object$up
  env$theta <- p
