...
plotConf <- function(
    ...
    if (!is.null(var1) && nn == var1) {
      ...
      if (is.factor(v)) {
        if (nn %in% var2) {
          newdata <- cbind(
            newdata,
            factor(rep(levels(v), each = npoints), levels = thelevels)
          )
          partdata[, nn] <- factor(
            rep(levels(v)[1], nrow(partdata)),
            levels = levels(v)
          )
        } else {
          newdata <- cbind(
            newdata,
            factor(rep(levels(v)[1], k * npoints), levels = levels(v))
          )
        }
      } else {
        if (is.logical(v)) {
  ...
  bb0 <- bb
  bb0[notbidx] <- 0
  myse <- apply(XX[, bidx, drop = FALSE], 1, function(x) {
    rbind(x) %*% SS[bidx, bidx, drop = FALSE] %*% cbind(x)
  })^.5
  ci.all <- list(fit = XX %*% bb0, se.fit = myse)
  z <- qnorm(1 - (1 - level) / 2)
  ci.all$fit <- cbind(
    ci.all$fit,
    ci.all$fit - z * ci.all$se.fit,
    ci.all$fit + z * ci.all$se.fit
  )
  ...
  if (inherits(model, "lmerMod")) {
    uz <- as.matrix(
      unlist(lme4::ranef(model)) %*%
        do.call(rbind, lme4::getME(model, "Ztlist"))
    )[1, ]
    R <- R - uz
  }
  ...
  if (!plot) {
    return(list(x = x, y = pr, predict = ci.all, predict.newdata = newdata))
  }
  ...
  if (is.null(var1)) {
    ...
    if (inherits(model, c("lm", "glm"))) {
      ...
      myconf <- myconf +
        qnorm(0.975) * cbind((diag(as.matrix(SS))[-1])^0.5) %x% cbind(-1, 1)
    }
    for (pos in seq(k)) {
      if (cidiff) {
        if (pos > 1) {
          ...
          yl <- ci0[1]
          yu <- ci0[2]
          y <- trans(intercept + mycoef[pos - 1])
        } else {
          yu <- yl <- NULL
          y <- trans(intercept)
        }
        ...
        ci0 <- rbind(trans(ci.all$fit[
          (npoints * (pos - 1) + 1):(pos * npoints),
        ]))
        y <- ci0[, 1]
        yu <- ci0[, 3]
        yl <- ci0[, 2]
