...
PD <- function(
  ...
  if (length(intercept) < length(beta)) {
    ...
    if (!missing(x)) {
      if (length(x) != N) {
        stop(
          "x should be of same length as the total length of 'intercept','slope','prob'"
        )
      }
  ...
  if (!is.null(prob)) {
    if (length(prob) < length(beta)) {
      ...
      if (!missing(x)) {
        B.prob[prob] <- x[
          length(intercept) + length(slope) + seq_len(length(prob))
        ]
      } else {
        B.prob[prob] <- 1
      }
  ...
  f <- function(b) {
    ...
    if (!is.null(EB)) {
      if (is.null(prob)) {
        stop("Index of mixture-probability parameters needed")
      }
...
zibreg <- function(
  ...
  beta <- op$par[beta.idx]
  gamma <- op$par[gamma.idx]
  cc <- c(beta, gamma)
  names(cc) <- c(colnames(X), paste0("pr:", colnames(Z)))
  bread <- Inverse(zibreg_information(
    beta,
    gamma,
    y,
    X,
...
predict.zibreg <- function(
  ...
  newf <- as.formula(paste(
    "~",
    paste(deparse(object$formula[[3]]), collapse = " ")
  ))
  if (missing(newdata)) {
    X <- object$X
    Z <- object$Z
...
summary.zibreg <- function(object, level = 0.95, pr.contrast, ...) {
  ...
  pr.names <- unlist(lapply(rownames(cc)[object$gamma.idx], function(x) {
    substr(x, 4, nchar(x))
  }))
  ...
  pr.cc <- cbind(
    pr.contrast %*% cc[object$gamma.idx, 1],
    diag(
      (pr.contrast) %*%
        vcov(object)[object$gamma.idx, object$gamma.idx] %*%
        t(pr.contrast)
    )^0.5
  )
  pr.cc <- object$family$linkinv(cbind(
    pr.cc[, 1],
    pr.cc[, 1] - qq * pr.cc[, 2],
    pr.cc[, 1] + qq * pr.cc[, 2]
  ))
  ...
  return(structure(
    list(coef = cc, prevalence = pr.cc),
    class = "summary.zibreg"
  ))
}
...
zibreg_logL <- function(
  ...
  structure(
    loglik,
    nobs = n,
    df = length(beta) + length(gamma),
    class = "logLik"
  )
}

##' @export
score.zibreg <- function(
  x,
  beta = x$beta,
  gamma = x$gamma,
  data,
  ...
  if (indiv) {
    colnames(s) <- names(x$coef)
  } else {
    names(s) <- names(x$coef)
  }
  return(s)
}

zibreg_score <- function(
  beta,
  gamma,
  y,
  X,
...
##' @export
information.zibreg <- function(
  x,
  beta = x$beta,
  gamma = x$gamma,
  data,
  offset = x$offset,
  type = c("robust", "outer", "obs"),
  ...
) {
  if (!missing(data)) {
    y <- model.frame(x$formula, data)[, 1]
...
zibreg_information <- function(
  ...
  if (tolower(type[1]) %in% c("obs", "hessian")) {
    ...
    I <- -numDeriv::jacobian(
      function(x) {
        zibreg_score(x[beta.idx], x[gamma.idx], y, X, Z, offset, family, ...)
      },
      c(beta, gamma)
    )
    return(I)
