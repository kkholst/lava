...
"distribution<-.lvm" <- function(
  ...
  if (!missing(value)) {
    for (obj in c("variable", "parname", "init", "mdist")) {
      if (
        !is.null(attr(value, obj)) &&
          eval(substitute(missing(a), list(a = obj)))
      ) {
  ...
  if (!is.null(parname) || length(dots) > 0) {
    if (length(parname) > 1 || (is.character(parname))) {
      ...
      gen <- value
      if ("..." %ni% names(formals(gen))) {
        formals(gen) <- c(formals(gen), alist(... = ))
      }
  ...
  if (length(variable) == 1 && !mdist) {
    ...
    if (!is.null(attributes(value)$mean)) {
      intercept(x, variable) <- attributes(value)$mean
    }
    if (!is.null(attributes(value)$variance)) {
      variance(x, variable, exo = TRUE) <- attributes(value)$variance
    }
    ...
    for (i in vars) {
      ...
      x$attributes$mdistribution$var[which(
        x$attributes$mdistribution$var == pos
      )] <- NULL
      above <- which(x$attributes$mdistribution$var > pos)
      if (length(above) > 0) {
        x$attributes$mdistribution$var[above] <- lapply(
          x$attributes$mdistribution$var[above],
          function(x) x - 1
        )
      }
    }
  ...
  if (
    is.list(value) &&
      length(value) == 1 &&
      (is.function(value[[1]]) || is.null(value[[1]]))
  ) {
    addvar(x) <- variable
    ## Multivariate distribution
    if (is.null(x$attributes$mdistribution)) {
      x$attributes$mdistribution <- list(var = list(), fun = list())
    }
    ...
    if (any(ii <- which(names(vars) %in% variable))) {
      ...
      for (i in seq_along(numleft)) {
        newfunlist <- c(
          newfunlist,
          x$attributes$mdistribution$fun[[numleft[i]]]
        )
...
"distribution.lvm" <- function(x, var, value, ..., multivariate = FALSE) {
  ...
  if (multivariate) {
    return(x$attributes$mdistribution)
  }
...
##' @export
lognormal.lvm <- function(...) {
  structure(
    normal.lvm(..., log = TRUE),
    family = list(family = "log-normal", ...)
  )
}

##' @export
poisson.lvm <- function(link = "log", lambda, ...) {
  fam <- stats::poisson(link)
  fam$link <- link
  f <- function(n, mu, ...) {
    if (missing(n)) {
      return(fam)
  ...
  if (!missing(lambda)) {
    attr(f, "mean") <- fam$linkfun(lambda)
  }
  attr(f, "family") <- fam
  attr(f, "var") <- FALSE
  return(f)
...
##' @export
pareto.lvm <- function(lambda = 1, ...) {
  ## shape: lambda, scale: mu
  ...
  attr(f, "family") <- list(family = "pareto", par = c(lambda = lambda))
  return(f)
}

##' @export
threshold.lvm <- function(p, labels = NULL, ...) {
  if (sum(p) > 1 || any(p < 0 | p > 1)) {
    stop("wrong probability vector")
  }
  if (!is.null(labels)) {
    return(function(n, ...) {
      return(cut(
        rnorm(n),
        breaks = c(-Inf, qnorm(cumsum(p)), Inf),
        labels = labels
      ))
    })
  }
...
Gamma.lvm <- function(
  ...
  if (!missing(shape) & missing(rate)) {
    if (unit) {
      ...
      f <- function(n, mu, var, ...) {
        rgam(n, shape = shape, rate = sqrt(shape / var))
      }
    } else {
      f <- function(n, mu, var, ...) {
        rgam(n, shape = shape, rate = shape / fam$linkinv(mu))
      }
    }
  }
  if (missing(shape) & !missing(rate)) {
    if (unit) {
      ...
      f <- function(n, mu, var, ...) {
        rgam(n, shape = rate * fam$linkinv(mu), rate = rate)
      }
    }
  }
  if (missing(shape) & missing(rate)) {
...
weibull.lvm <- function(intercept = 0, sigma = .5, scale, shape) {
  ...
  f <- function(n, mu = 0, var, ...) {
    (-log(runif(n)) * exp(log(scale) * shape) * exp(mu * shape))^{
      1 / shape
    }
...
GM3.lvm <- function(
  ...,
  parname = c("Pr1", "Pr2", "M1", "M2", "M3", "V1", "V2", "V3"),
  init = c(0.25, 0.5, -4, 0, 4, 1, 1, 1)
) {
