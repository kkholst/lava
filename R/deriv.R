...
deriv.function <- function(
  ...
  parameter_,
  ...,
  parameter_.increment = .Machine$double.xmin
) {
  p <- length(parameter_)
  f0 <- expr(parameter_)
  z0 <- numeric(p)
...
deriv.lvm <- function(
  ...
  if (zeroones | is.null(ii$dA)) {
    ...
    if (npar.reg > 0) {
      dA[, regr.idx] <- sapply(regr.idx, function(i) {
        izero(which(t(nn$A) == i), nrow(dA))
      })
    }
    if (npar > npar.reg) {
      dP[, var.idx] <- sapply(var.idx, function(i) {
        izero(which(nn$P == i), nrow(dA))
      })
    }
...

    {
      ...
      if (!is.null(meanpar) & npar.mean > 0) {
        dv[, mean.idx] <- sapply(mean.idx, function(i) {
          izero(which(nn$v == i), length(expr$mean))
        })
      }
    ...
    for (pp in index(expr)$constrain.par) {
      ...
      if (!is.null(myc)) {
        ...
        for (jj in seq_len(length(vals))) {
          allpars <- c(
            nn$A[attributes(vals[[jj]])$reg.idx[1]],
  ...
  if (!lava.options()$devel) {
    ...
    dG <- with(
      mom,
      kronprod(t(IAi), G, res$dA[, ii$parBelongsTo$reg, drop = FALSE])
    )
    ...
    dS[, ii$parBelongsTo$reg] <- G1 + G2
    dS[, ii$parBelongsTo$cov] <- G3
  }
    ...
    if (lava.options()$devel) {
      dG <- with(
        ...
        kronprod(
          t(IAi),
          G,
          res$dA[, with(ii$parBelongsTo, c(mean, reg)), drop = FALSE]
        )
      )
    }
    dxi <-
