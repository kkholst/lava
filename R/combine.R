excoef <- function(
  ...
  pval[which(pvalround < 10^(-p.digits))] <- paste0(
    "p<0.",
    paste(rep("0", p.digits - 1), collapse = ""),
    "1"
  )
  res <- cbind(res, pval)
  ...
  for (i in seq(nrow(res))) {
    row <- paste0(
      if (res[i, 1] >= 0) " " else "",
      res[i, 1],
      paste(rep(" ", nc[i, 1]), collapse = ""),
      if (res[i, 1] < 0) " ",
      if (se) {
        paste0(" (", res[i, 2], ")", paste(rep(" ", nc[i, 2]), collapse = ""))
      },
      if (ci) {
        paste0(
          " [",
          ...
          res[i, 5],
          "]",
          paste(rep(" ", nc[i, 4] + nc[i, 5]), collapse = "")
        )
      },
