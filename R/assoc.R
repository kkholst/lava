...
prob.normal <- function(sigma, breaks, breaks2 = breaks) {
  ...
  if (ncol(sigma) != 2 || missing(breaks)) {
    stop("Wrong input")
  }
  ...
  for (i in seq(length(breaks) - 1)) {
    for (j in seq(length(breaks2) - 1)) {
      P[i, j] <- mets::pmvn(
        lower = c(breaks[i], breaks2[j]),
        upper = c(breaks[i + 1], breaks2[j + 1]),
        sigma = sigma
      )
    }
  }
...
OR <- function(x, tabulate = FALSE, log = FALSE, ...) {
  ...
  pos <- M$position
  if (ncol(pos) != 2 & ncol(pos) != 2) {
    stop("Only for 2x2 tables")
  }
  orfun <- function(p, ...) {
    list(logOR = sum(log(p[diag(pos)])) - sum(log(p[revdiag(pos)])))
  }
  summary(estimate(M, orfun), transform = exp)
}
...
information_assoc <- function(P, base = exp(1), ...) {
  ...
  for (j in seq_along(P.col)) {
    if (P.col[j] > 0) {
      H.col <- H.col - P.col[j] * log(P.col[j] + (P.col[j] == 0), base = base)
    }
  }
  for (i in seq_along(P.row)) {
    if (P.row[i] > 0) {
      H.row <- H.row - P.row[i] * log(P.row[i] + (P.row[i] == 0), base = base)
    }
    for (j in seq_along(P.col)) {
...
independence <- function(x, ...) {
  ...
  if (!inherits(x, "multinomial")) {
    stop("Expected table, data.frame or multinomial object")
  }
  if (length(x$levels) != 2) {
    stop("Data from two categorical variables expected")
  }
  f <- function(p) {
    P <- x$position
    P[] <- p[x$position]
  ...
  e <- estimate(
    ...
    print = function(x, ...) {
      ...
      cat(
        "Test statistc:\t ",
        formatC(x$coefmat[1] / x$coefmat[2]),
        "\nP-value:\t ",
        x$coefmat[5],
        "\n\n"
      )
