...
spaghetti <- function(
  ...
  if (!lava.options()$cluster.index) {
    stop("mets not available? Check 'lava.options()cluster.index'.")
  }
  if (!is.null(by)) {
  ...
  if (!is.null(group)) {
    ...
    if (!add) {
      plot(formula, data = data, xlab = xlab, ylab = ylab, ..., type = "n")
    }
    dd <- split(data, M)
    K <- length(dd)
    if (length(type) < K) {
    ...
    if (length(trend.col) < K) {
      trend.col <- rep(trend.col, K)
    }
    if (length(trend.lty) < K) {
      trend.lty <- rep(trend.lty, K)
    }
    if (length(trend.alpha) < K) {
      trend.alpha <- rep(trend.alpha, K)
    }
    if (length(trend.lwd) < K) {
      trend.lwd <- rep(trend.lwd, K)
    }
    for (i in seq_len(K)) {
      spaghetti(
        formula,
  ...
  if (length(x) == 0) {
    ...
    wide <- mets::fast.reshape(
      data[order(data[, id], data[, x]), ],
      id = id,
      varying = c(y, x),
      ...
    )
    ...
    if (trend) {
      if (is.numeric(trend.formula)) {
        ...
        res <- c()
        if (!is.null(tau)) {
          if (length(trend.alpha) < length(tau)) {
            trend.alpha <- rep(trend.alpha, length(tau))
          }
          if (length(trend.lty) < length(tau)) {
            trend.lty <- rep(trend.lty, length(tau))
          }
          if (length(trend.col) < length(tau)) {
            trend.col <- rep(trend.col, length(tau))
          }
          if (length(trend.lwd) < length(tau)) {
            trend.lwd <- rep(trend.lwd, length(tau))
          }
        }
