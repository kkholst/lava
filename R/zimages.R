img <- function(x,idx,col=list(gray.colors(10,1,0.2)),
    ...
    if (axis1) axis(1,at=seq(nrow(x)),lwd=0.5,cex.axis=cex.axis,las=3)
    if (axis2) axis(2,at=seq(ncol(x)),lwd=0.5,cex.axis=cex.axis,las=1)
    if (!missing(idx)) {
...
## X <- matrix(rbinom(400,4,0.5),20)
## group <- rep(1:4,each=5)
## images(X,colorbar=0)
## images(X,group=group)
## images(X,group=group,col=list(brewer.pal(4,"Purples"),
##                                brewer.pal(4,"Greys"),
##                                brewer.pal(4,"YlGn"),
##                                brewer.pal(4,"PuBuGn")),colorbar=2)
## images(list(X,X,X,X),group=group)
## images(list(X,X,X,X),ncol=1,group=group)
...
images <- function(x,group,ncol=2,byrow=TRUE,colorbar=1,colorbar.space=0.1,label.offset=0.02,
    ...
    for (i in seq(length(x))) {
        ...
        img(x[[i]],idxs,col,cex.axis=0.6,axis2=axis2[i],axis1=axis1[i],main=main[i],...)
