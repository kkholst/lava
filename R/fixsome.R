...
fixsome <- function(
  ...
  if (measurement.fix & param != "none") {
    ...
    for (e in etas) {
      ## Makes sure that at least one arrow from latent variable is fixed (identification)
      ys. <- names(which(M[e, ys] == 1))
      if (length(ys.) > 0) {
        if (tolower(param) == "absolute") {
          if (is.na(intercept(x)[[e]])) {
            intercept(x, e) <- 0
          }
          if (is.na(x$covfix[e, e]) & is.na(x$covpar[e, e])) {
            covariance(x, e) <- 1
          }
        } else {
          if (param == "hybrid") {
            ...
            if (
              !any(unlist(lapply(intercept(x)[ys.], is.numeric))) &
                is.na(intercept(x)[[e]])
            ) {
              if (
                tryCatch(
                  any(idx <- !is.na(x$fix[e, ys.])),
