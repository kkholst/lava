...
`finalize.lvm` <-
  function(
    ...
    for (i in seq_len(length(x$noderender))) {
      ...
      if (length(nn) > 0) {
        R <- list(as.list(x$noderender[[i]]))
        names(R) <- names(x$noderender)[i]
    ...
    if (!is.null(Lab)) {
      ## Ugly hack to allow mathematical annotation
      nn <- names(graph::nodeRenderInfo(g)$label)
      LL <- as.list(graph::nodeRenderInfo(g)$label)
      LL[names(Lab)] <- Lab
      if (
        any(unlist(lapply(LL, function(x) {
          is.expression(x) || is.name(x) || is.call(x)
        })))
      ) {
        graph::nodeRenderInfo(g) <- list(label = as.expression(LL))
      } else {
        graph::nodeRenderInfo(g) <- list(label = LL)
      }
    ...
    if (unexpr) {
      mylab <- as.character(graph::edgeRenderInfo(g)$label)
      names(mylab) <- names(graph::edgeRenderInfo(g)$label)
    ...
    if (index(x)$npar.reg > 0) {
      for (i in seq_len(nrow(A) - 1)) {
        for (j in (i + 1):(ncol(A))) {
          ...
          if (A[i, j] == 1) {
            regEdges <- c(regEdges, paste0(var[i], "~", var[j]))
          }
    ...
    if (cor | diag) {
      for (r in seq_len(nrow(covariance(x)$rel) - delta)) {
        for (s in (r + delta):ncol(covariance(x)$rel)) {
          if (cor | r == s) {
            if (
              covariance(x)$rel[r, s] == 1 &
                (!any(c(var[r], var[s]) %in% exogenous(x)))
            ) {
              newedges <- c()
              if (A[r, s] != 1) {
    ...
    if (length(x$edgerender$futureinfo) > 0) {
      ...
      revestr <- sapply(estr, function(y) {
        paste(rev(unlist(strsplit(y, "~"))), collapse = "~")
      })
      ...
      for (i in estr) {
        ...
        for (f in names(x$edgerender$futureinfo)) {
          if (count %in% revidx) {
            g@renderInfo@edges[[f]][[revestr[
              count
            ]]] <- x$edgerender$futureinfo[[f]][[i]]
          } else {
    ...
    for (e in allEdges) {
      ...
      for (f in c("col", "cex", "textCol", "lwd", "lty")) {
        if (
          !(estr %in% names(graph::edgeRenderInfo(g)[[f]])) ||
    ...
    if (plain) {
      ...
      if (addcolor) {
        if (is.null(x$noderender$fill)) {
          notcolored <- vars(x)
        } else {
          notcolored <- vars(x)[is.na(x$noderender$fill)]
        }
