...
correlation.lvmfit <- function(
  ...
  pp <- matrices2(
    Model(x),
    with(index(x), seq_len(npar + npar.mean + npar.ex))
  )$P
  ...
  res <- estimate(x, coef = coef(x), vcov = V, f = ff, IC = IC)
  if (back.transform) {
...
correlation.matrix <- function(
  ...
  for (i in seq(ncol(x) - 1)) {
    for (j in seq(i + 1, ncol(x))) {
      e <- c(
        e,
        list(correlation(x[, c(i, j)], z = z, back.transform = FALSE, ...))
      )
