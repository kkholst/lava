...
totaleffects.lvmfit <- function(object, to, ..., level = 0.95) {
  ...
  if (is.null(list(...)$from) & is.character(to)[1]) {
    to <- lapply(
      paste(to, setdiff(vars(object), to), sep = "~"),
      FUN = as.formula
    )
  }
  ef <- function(tt) {
    ...
    rbind(with(
      ...
      c(
        ...
        sd,
        est / sd,
        2 * (pnorm(abs(est / sd), lower.tail = FALSE)),
        est + q * sd,
        est - q * sd
      )
    ))
...
effects.lvmfit <- function(object, to, from, ...) {
  ...
  S <- matrix(0, nrow(cc), nrow(cc))
  rownames(S) <- colnames(S) <- rownames(cc)
  V <- object$vcov
  ...
  for (i in seq_len(length(idx.list))) {
    K <- length(idx.list[[i]])
    idx.list[[i]] <- idx.all[pos:(pos + K - 1)]
    pos <- pos + K
  }
  margef <- list()
  if (length(coefs.all) == 1 && is.na(coefs.all)) {
    ...
    for (i in seq_len(length(idx.list))) {
      margef <- c(
        margef,
        list(prodsumdelta(coefs.all, idx.list[i], S.all, ...))
      )
    }
  }
...
summary.effects <- function(object, ...) {
  ...
  for (i in K) {
    if (length(object$paths[[i]]) > 2) {
      nn <- c(
        nn,
        paste(rev(object$paths[[i]]), collapse = lava.options()$symbol[1])
      )
  ...
  if (totindirectef[1] != 0) {
    if (abs(coef(object)[2]) < 1e-12) {
      medprop <- estimate(
        NULL,
        coef = c("Mediation proportion" = 1),
        vcov = matrix(NA)
      )
    } else {
      medprop <- estimate(object, function(x) {
        list("Mediation proportion" = 1 - x[2] / x[1])
      })
    }
  }
...
prodsumdelta <- function(betas, prodidx, S, order = 1) {
  ## Delta-method
  k <- length(prodidx)
  p <- length(betas)
  if (p == 1) {
