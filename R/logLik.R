  ...
  if (
    ...
    myfun <- function(ii) {
      ...
      return(logLikFun(
        ...
        p = p,
        weights = weights[ii, , drop = FALSE],
        data2 = data2[ii, , drop = FALSE],
        model = model,
        debug = debug,
        indiv = indiv,
        ...
  ...
  if (xconstrainM) {
    ...
    for (i in seq_len(length(constrain(object)))) {
      ...
      if (length(xx) > 0) {
        ...
        y <- names(which(unlist(lapply(intercept(object), function(x) {
          x == parname
        }))))
    ...
    if (length(xconstrain) > 0) {
      ...
      Mu <- matrix(0, nrow(data), length(vars(object)))
      colnames(Mu) <- vars(object)
      M <- modelVar(object, p = p, data = data)
      M$parval <- c(
        M$parval,
        object$mean[unlist(lapply(object$mean, is.numeric))]
      )
...
gaussian_logLik.lvm <- function(
  ...
  if (!lava.options()$allow.negative.variance && any(diag(mom$P) < 0)) {
    return(NaN)
  }
...
##' @export
logLik.multigroup <- function(
  object,
  p,
  data = object$data,
  weights = NULL,
  type = c("cond", "sim", "exo", "sat"),
  ...
) {
