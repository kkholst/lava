...
density.sim <- function(x, ..., add = FALSE, plot.type = "single") {
  plot.sim(x, ..., add = add, scatter.plot = FALSE, plot.type = plot.type)
}
...
plot.sim <- function(
  ...
  if (equal || single) {
    ...
    if (density.plot) {
      if (is.null(ylim)) {
        ylim <- rep(list(c(0, maxdy * delta)), K)
      }
  ...
  if (!is.null(scatter.ylim)) {
    if (!is.list(scatter.ylim)) {
      scatter.ylim <- list(scatter.ylim)
    }
  ...
  my.scatter.sim <- function(i, add = FALSE, colors, ...) {
    ...
    if (!is.null(scatter.ylim)) {
      args <- c(args, list(ylim = scatter.ylim[[i]]))
    }
    if (!is.null(scatter.xlim)) {
      args <- c(args, list(xlim = scatter.xlim[[i]]))
    }
    if (scatter.plot) {
      if (!add) {
  ...
  my.density.sim <- function(
    ...
    if (density.plot) {
      ...
      if (polygon) {
        ...
        if (!is.null(border)) {
          with(dy, lines(x, y, col = border[1], lty = lty[1], lwd = lwd[1]))
        }
      } else {
        graphics::lines(dy, main = "", lty = lty[1], col = col[1], lwd = lwd[1])
      }
      if (rug) {
        graphics::rug(y, col = Col(col[1], rug.alpha[1]))
      }
      ...
      if (!is.null(se) && !any(is.na(se[[i]]))) {
        ...
        for (j in seq_along(se.pos)) {
          if (polygon) {
            yy <- dnorm(
              xx,
              mean = ss["Mean", se.pos[j]],
              sd = ss["SE", se.pos[j]]
            )
            ## Mirror the same fill logic as the empirical polygon
            se.fill <- if (
              is.null(densities[j + 1]) || is.na(densities[j + 1])
            ) {
            ...
            if (!is.null(border)) {
              lines(xx, yy, col = se.col[j], lty = se.lty[j], lwd = se.lwd[j])
            }
          } else {
            graphics::curve(
              dnorm(x, mean = ss["Mean", se.pos[j]], sd = ss["SE", se.pos[j]]),
              lwd = se.lwd[j],
              lty = se.lty[j],
              col = se.col[j],
              add = TRUE
        ...
        if (auto.legend) {
          legend <- c("Empirical", colnames(ss)[se.pos])
        }
        if (!is.null(legend)) {
          if (polygon) {
            dcol <- c(col[1], se.col)
  ...
  if (single) {
    ...
    for (i in seq_len(K)) {
      ...
      my.density.sim(
        i,
        add = if (i > 1) TRUE else add, # first panel respects add
        colors = col[i],
        alphas = alphas,
        densities = density,
        angles = angle,
