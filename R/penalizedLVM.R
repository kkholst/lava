...
penalised_method.lvm <- "nlminb2"#lava:::gaussian_method.lvm
...
penalised_objective.lvm <- function(x, ...){  # proportional to the log likelihood
  ...
  if(!is.null(penalisation)){
    ...
    obj.P <- penalisation$fn(coef = dots$p[penalisation$index.coef],
...
penalised_gradient.lvm <- function(x, ...){
  ...
  if(!is.null(penalisation)){
    res_tempo <- penalisation$gn(coef = dots$p[penalisation$index.coef],
                                 lambda1 = penalisation$lambda1, 
                                 lambda2 = penalisation$lambda2)

  grad.P <- rep(0, length(dots$p))
  grad.P[penalisation$index.coef] <- res_tempo
  ...
  if(length(grad.UP)-length(grad.P) !=0){browser()}
...
penalised_hessian.lvm <- function(x, ...){ # second order partial derivative
  ...
  if(!is.null(penalisation)){
    ...
    res_tempo <- penalisation$hn(coef = dots$p[penalisation$index.coef],
                                 lambda1 = penalisation$lambda1, 
                                 lambda2 = penalisation$lambda2)
...
ls.fct_penalty <- function(lvm, lambda1 = 0, lambda2 = 0, names.coef = NULL,
  ...
  if(is.null(names.coef)){
    ...
    index.coef <- which(coef(lvm) %in% names.coef)
    if(any(names.coef %in% coef(lvm) == FALSE)){
      stop("ls.fct_penalty: coefficient to be penalized do not match those of the model\n",
           "unknown coefficient: ",names.coef[names.coef %in% coef(lvm) == FALSE],"\n",
           "available coefficients: ",coef(lvm)[coef(lvm) %in% names.coef == FALSE],"\n")
    }
  }
