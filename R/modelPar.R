...
modelPar.lvm <- function(x, p, ...) {
  ...
  if (length(p) != npar & length(p) < (npar + npar.mean)) {
    stop("Wrong dimension of parameter vector!")
  }
  p2 <- NULL
  if (length(p) != npar) {
    ## if meanstructure
...
modelPar.multigroup <- function(x, p, ...) {
  ...
  if (length(pp) == 0) {
    pp <- lapply(seq_len(x$ngroup), function(x) logical())
  }
  mm <- lapply(x$meanposN, function(x) p[x])
  if (is.null(mm)) {
    mm <- lapply(seq_len(x$ngroup), logical())
  }
  pm <- mm
  for (i in seq_len(length(pm))) {
    pm[[i]] <- c(pm[[i]], pp[[i]])
  }
...
modelPar2.multigroup <-
  function(x, p, ...) {
    ...
    parlist <- list()
    for (i in seq_len(k)) {
      parlist[[i]] <- numeric(length(x$parlist[[i]]))
    }
    if (!is.null(meanpar)) {
      meanlist <- list()
      for (i in seq_len(k)) {
        meanlist[[i]] <- numeric(length(x$meanlist[[i]]))
      }
    }
