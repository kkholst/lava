...
parpos.multigroup <- function(x, p, mean = TRUE, ...) {
  if (missing(p)) {
    p <- unique(unlist(lapply(x$lvm, function(z) {
      setdiff(parlabels(z), names(constrain(z)))
    })))
  }
  if (!is.character(p)) {
    p <- names(p)
...
parpos.lvm <- function(x, p, mean = TRUE, ...) {
  ...
  if (mean) {
    ...
    nn <- with(index(x), matrices2(x, seq_len(npar + npar.mean + npar.ex)))
  } else {
    nn <- with(
      index(x),
      matrices(x, seq_len(npar), NULL, seq_len(npar.ex) + npar)
    )
  }
  nn$A[index(x)$M0 != 1] <- 0
  nn$P[index(x)$P0 != 1] <- 0
