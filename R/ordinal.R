...
ordinal_estimate_hook <- function(x, data, weights, data2, estimator, ...) {
  ...
  ord <- ordinal(x)
  bin <- NULL
  if (is.null(estimator) && length(ord) > 0) {
  ...
  if (is.null(estimator) || estimator %in% nestimator[1]) {
    for (i in setdiff(lava::endogenous(x), c(bin, ord))) {
      if (survival::is.Surv(data[, i])) {
        ...
        if (
          !(attributes(S)$type %in% c("left", "right", "interval2", "interval"))
        ) {
          stop("Surv type not supported.")
        }
        mynames <- c(mynames, i)
        y2 <- cbind(y2)
        colnames(y2) <- i
...
print.ordinal.lvm <- function(x, ...) {
  ...
  for (i in x) {
    ...
    if (length(val) == 0) {
      ...
      print(unlist(attr(x, "fix")[[i]]), quote = FALSE)
    }
    cat(rep("_", 28), "\n", sep = "")
  }
}
...
`ordinal.lvm` <- function(
  ...
  if (length(var) > length(K)) {
    K <- rep(K[1], length(var))
  }
  if (length(var) == 1 && !missing(constrain)) {
    constrain <- list(constrain)
  }
  if (length(var) > 1) {
    if (!missing(labels) && !is.list(labels)) {
      labels <- rep(list(labels), length(var))
    }
    if (!missing(breaks) && !is.list(breaks)) {
      breaks <- rep(list(breaks), length(var))
    }
    if (!missing(constrain) && !is.list(constrain)) {
      constrain <- rep(list(constrain), length(var))
    }
  }
  ...
  for (i in seq_len(length(var))) {
    if (K[i] > 2 || (K[i] == 2 && !liability)) {
      parname <- paste0(
        var[i],
        ":",
        paste(seq(K[i] - 1) - 1, seq(K[i] - 1), sep = "|")
      )
      ...
      if (length(newpar) < K[i] - 1) {
        stop("Wrong number of starting values")
      }
      newfix <- if (missing(constrain)) {
        rep(list(NA), length(newpar))
      } else {
        constrain[[i]]
      }
      if (length(newfix) < K[i] - 1) {
        stop("Wrong number of constraints")
      }
