  ...
  function(object, ...) {
    ...
    if (length(index(object)) > 0) {
      cat(
        ...
        with(index(object), npar + npar.mean + npar.ex),
        "\n",
        sep = ""
      )
    }
...
print.summary.lvmfit <- function(x, varmat = TRUE, ...) {
  if (!is.null(x$control$method)) {
    ...
    rnkV <- qr(x$vcov)$rank
    if (l2D > 1e-2) {
      warning("Possible problems with convergence!")
    }
    cat("||score||^2=", l2D, "\n", sep = "")
    np <- nrow(x$vcov)
    if (rnkV < np) {
      warning(
        "Possible problems with identification (rank(informaion)=",
        rnkV,
        "<",
        np,
        "!"
...
summary.multigroupfit <- function(object, groups = NULL, ...) {
  ...
  res <- list(
    coef = coef(object, type = 2, groups = groups, ...),
    object = object,
    coefmat = cc,
    gof = gof(object),
    object = object,
