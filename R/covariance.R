...
"covariance<-.lvm" <- function(
  ...
  if (inherits(value, "formula")) {
    ...
    covariance(object, value, pairwise = pairwise, ...)
  }
}
...
`covariance.lvm` <-
  function(
    ...
    if (!is.null(var1)) {
      ...
      if (constrain) {
        if (length(allvars) != 2) {
          stop("Constraints only implemented for pairs")
        }
...
covarianceconst <- function(
  ...
  nvarname <- c("rname", "cname", "vname", "v2name", "lname", "l2name")[is.na(c(
    rname,
    cname,
    vname,
    v2name,
  ...
  constrain(object, cname, cpar) <- function(x) {
    prod(exp(x[seq(length(cpar) - 1)]))^(1 / (length(cpar) - 1)) *
      tanh(x[length(cpar)])
  }
