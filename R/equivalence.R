...
equivalence <- function(x, rel, tol = 1e-3, k = 1, omitrel = TRUE, ...) {
  if (missing(rel)) {
    stop("Specify association 'rel' (formula or character vector)")
  }
  ...
  relname <- c(
    paste(myvars, collapse = lava.options()$symbol[2]),
  ...
  if (omitrel) {
    ## Don't save models including 'rel'
    keep <- c()
    if (length(Equiv) > 0) {
      for (i in seq_len(length(Equiv))) {
        newvars <- s$var[[Equiv[i]]]
        if (!any(apply(newvars, 1, function(z) all(z %in% myvars)))) {
          keep <- c(keep, Equiv[i])
        }
      }
      Equiv <- keep
    ...
    for (i in seq_len(length(Equiv))) {
      ...
      for (j in seq_len(nrow(newvars))) {
        ...
        if (length(exo.idx) > 0) {
          xnew <- regression(
            xnew,
            from = newvars[j, exo.idx],
            to = newvars[j, setdiff(1:2, exo.idx)]
          )
        } else {
...
print.equivalence <- function(x, ...) {
  cat(
    "  0)\t ",
    paste0(x$vars, collapse = lava.options()$symbol[2]),
    "  (",
    formatC(x$T0),
    ")\n"
  )
