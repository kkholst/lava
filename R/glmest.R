...
GLMest <- function(m, data, control = list(), ...) {
  ...
  for (y in yvar) {
    ...
    if (length(p) > 1) {
      ...
      if (length(p) > length(xx0) + 1) {
        names(p)[length(p)] <- paste(y, y, sep = lava.options()$symbol[2])
      }
    }
    if (
      tolower(fam$family) %in%
      ...
      ics <- cbind(ics, 0)
      null <- matrix(0)
      dimnames(null) <- list("scale", "scale")
  ...
  colnames(mymsg) <- "Family(Link)"
  rownames(mymsg) <- paste(yvar, ":")
  list(
    estimate = coefs,
    vcov = V,
    breads = breads,
...
GLMscore <- function(x, p, data, indiv = TRUE, logLik = FALSE, ...) {
  ...
  for (y in yvar) {
    ...
    pname <- c(
      y,
      paste0(y, sep = lava.options()$symbol[1], xx),
      paste(y, y, sep = lava.options()$symbol[2])
    )
  ...
  attributes(S1)$logLik <- structure(
    L,
    nobs = nrow(data),
    nall = nrow(data),
    df = length(p),
...
score.lm <- function(
  ...
  if (any(is.na(p))) {
    warning("Over-parameterized model")
  }
  Xbeta <- X %*% p
  if (is.null(offset)) {
    offset <- x$offset
...
predict_glm <- function(
  ...
  if (missing(data)) {
    ...
    X <- stats::model.matrix.lm(formula(x), data = data, na.action = na.pass)
  }
...
score.glm <- function(
  ...
  if (pearson) {
    attr(S, "pearson") <- rpearson
  }
  suppressWarnings(attributes(S)$bread <- vcov(x) * NROW(S))
  if (
    x$family$family == "quasi" &&
      x$family$link == "identity" &&
      x$family$varfun == "constant"
  ) {
    attributes(S)$bread <-
      suppressWarnings(
