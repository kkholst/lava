...
pcor <- function(x, y, X, start, ...) {
  ...
  if (is.numeric(x) && is.numeric(y)) {
    ...
    return(estimate(
      e,
      function(p) list(rho = p[5] / (p[3] * p[4])^.5),
      IC = TRUE
    ))
  }
  ...
  ii <- mets::fast.pattern(
    cbind(as.numeric(x), as.numeric(y)),
    categories = max(length(unique(x)), length(unique(y)))
  )
  ...
  names(cc) <- c(
    "rho",
    paste(rownames(nn), "x", sep = ".")[-1],
    paste(colnames(nn), "y", sep = ".")[-1]
  )
...
polycor0 <- function(rho, a0, b0, onlyP = TRUE, ...) {
  ...
  for (k in seq(k1)) {
    for (i in seq(k1 + 1)) {
      for (j in seq(k2 + 1)) {
        pos <- i + (k1 + 1) * (j - 1)
        Da[pos, k] <- dpmvn0(i, j, type = 2, k = k) +
          dpmvn0(i - 1, j - 1, type = 2, k = k) -
          dpmvn0(i - 1, j, type = 2, k = k) -
          dpmvn0(i, j - 1, type = 2, k = k)
      }
    }
  }
  ...
  for (k in seq(k2)) {
    for (i in seq(k1 + 1)) {
      for (j in seq(k2 + 1)) {
        pos <- i + (k1 + 1) * (j - 1)
        Db[pos, k] <- dpmvn0(i, j, type = 3, k = k) +
          dpmvn0(i - 1, j - 1, type = 3, k = k) -
          dpmvn0(i - 1, j, type = 3, k = k) -
          dpmvn0(i, j - 1, type = 3, k = k)
      }
    }
  }
