...
`estimate.lvm` <-
  function(
    ...
    ## Random-slopes:
    xfix <- setdiff(
      colnames(data)[(colnames(data) %in% parlabels(x, exo = TRUE))],
      latent(x)
    )
    if (base::missing(fix)) {
    ...
    if (length(index(x)$endogenous) == 0) {
      stop(
        "No observed outcome variables. Check variable names in model and data."
      )
    }
    ...
    nparall <- index(x)$npar +
      ifelse(Optim$meanstructure, index(x)$npar.mean + index(x)$npar.ex, 0)
    ## Get starting values
    if (!missing(p)) {
      start <- p
      Optim$start <- p
      ...
      if (!is.null(Optim$start)) {
        ...
        for (i in which(paragree)) {
          count <- count + 1
          paragree_pos[count, ] <- c(
            i,
            which(myparnames[i] == names(Optim$start))[1]
          )
        }
      }
      ...
      if (!(length(Optim$start) == length(myparnames) & sum(paragree) == 0)) {
        ...
        start <- suppressWarnings(do.call(
          Optim$starterfun,
          list(
            x = x,
            S = S,
            mu = mu,
            debug = lava.options()$debug,
            messages = messages,
            data = data,
            ...
          )
        ...
        if (!is.null(x$expar) && length(start) < nparall) {
          ...
          start <- c(
            start,
            structure(unlist(x$expar[ii]), names = names(x$expar)[ii])
          )
        }
    ...
    if (
      length(xfix) > 0 |
        (length(xconstrain) > 0 & XconstrStdOpt | !lava.options()$test)
    ) {
      ## Yes
      x0 <- x
      ...
      if (length(xfix) > 0) {
        ...
        new.par.idx <- which(
          coef(mymodel, mean = TRUE, fix = FALSE) %in%
      ...
      myObj <- function(pp) {
        ...
        myfun <- function(ii) {
          if (length(xfix) > 0) {
            for (i in seq_along(myfix$var)) {
              x0$fix[cbind(rowpos[[i]], colpos[[i]])] <- index(x0)$A[cbind(
                rowpos[[i]],
                colpos[[i]]
              )] <- data[ii, xfix[i]]
            }
      ...
      myInfo <- function(pp) {
        ...
        val <- apply(
          array(unlist(L), dim = c(length(pp), length(pp), nrow(data))),
          c(1, 2),
          sum
        )
        if (!is.null(attributes(L[[1]])$grad)) {
          attributes(val)$grad <- colSums(
            matrix(
              unlist(lapply(L, function(i) attributes(i)$grad)),
              ncol = length(pp),
              byrow = TRUE
            )
          )
      ...
      MkOffset <- function(pp, grad = FALSE) {
        if (length(xconstrain) > 0) {
          ...
          for (i in seq_len(length(xconstrain))) {
            ...
            D <- cbind(
              rbind(pp) %x% cbind(rep(1, nrow(Mu))),
              data[, xconstrain[[i]]$exo, drop = FALSE]
            )[, myidx, drop = FALSE]
    ...
    myHess <- function(pp) {
      ...
      if (Optim$constrain) {
        I0[constrained, -constrained] <- apply(
          I0[constrained, -constrained, drop = FALSE],
          2,
          function(x) x * pp[constrained]
        )
        I0[-constrained, constrained] <- t(I0[constrained, -constrained])
        if (sum(constrained) == 1) {
          I0[constrained, constrained] <- I0[constrained, constrained] *
            outer(pp[constrained], pp[constrained]) -
            D[constrained]
        } else {
          I0[constrained, constrained] <- I0[constrained, constrained] *
            outer(pp[constrained], pp[constrained]) -
    ...
    if (!missing(p)) {
      ...
      if (!is.null(Optim$method)) {
        ...
        if (Optim$method %in% c("optimx", "optim")) {
          ...
          names(optarg)[1] <- "par"
          if (is.null(optarg$upper)) {
            optarg$upper <- Inf
          }
          if (!is.null(optarg[["objective"]])) {
            names(optarg)[2] <- "fn"
          }
        ...
        if (!is.null(Optim$optimx.method)) {
          optarg$method <- Optim$optimx.method
        }
        opt <- do.call(Optim$method, optarg)
        ...
        if (!NoOptim) {
          opt <- do.call(
            ObjectiveFun,
            list(x = x, data = data, control = control, ...)
          )
          opt$gradient <- rep(0, length(opt$estimate))
        } else {
          opt <- list(
            estimate = Optim$start,
    ...
    if (NoOptim) {
      ...
      if (!exists(asVarFun)) {
        ...
        asVar <- tryCatch(Inverse(I), error = function(e) {
          matrix(NA, length(opt$estimate), length(opt$estimate))
        })
      } else {
        asVar <- tryCatch(
          do.call(
      ...
      if (any(is.na(asVar))) {
        warning(
          "Problems with asymptotic variance matrix. Possibly non-singular information matrix!"
        )
      }
