...
"edgelabels<-.lvmfit" <- function(
  ...
  if (est) {
    mytext <- paste(
      "c(",
      paste(
        paste0(edges., "=expression(", as.character(value), "==\"", val, "\")"),
        collapse = ","
      ),
      ")"
    )
    ...
    mytext <- paste(
      "c(",
      paste(
        paste0(edges., "=expression(", as.character(value), ")"),
        collapse = ","
      ),
      ")"
    )
  ...
  Par <-
    switch(
      ...
      sd = paste0(
        ...
        " (",
        do.call(format.fun, list(Par[, 2, drop = FALSE])),
        ")"
      ),
...
`edgelabels.graphNEL` <- function(
  ...
  if (!is.null(lab)) {
    if (!is.null(from) & !is.null(to)) {
      ...
      if (length(lab) != length(estr2)) {
        lab <- rep(lab, length(estr2))
      }
      if (length(col) != length(estr2)) {
        col <- rep(col, length(estr2))
      }
      if (length(cex) != length(estr2)) {
        cex <- rep(cex, length(estr2))
      }
      if (length(lwd) != length(estr2)) {
        lwd <- rep(lwd, length(estr2))
      }
      if (length(lty) != length(estr2)) {
        lty <- rep(lty, length(estr2))
      }
