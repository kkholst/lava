...
rho2sigma <- function(rho) {
  if (length(rho) == 1) {
    return(diag(2) * (1 - rho) + rho)
  }
  p <- introotpn(length(rho))
  if (is.na(p)) {
    stop("Unexpected length of correlation coefficients (p=n*(n-1)/2).")
  }
  sigma <- diag(nrow = p)
...
rmvn0 <- function(n, mu, sigma, rho, ...) {
  ...
  if (NROW(mu) == nrow(res) && NCOL(mu) == ncol(res)) {
    return(res + mu)
  }
...
normal_objective.lvm <- function(
  ...
  if (Table) {
    pat <- mets::fast.pattern(
      data[, y, drop = FALSE],
      categories = max(data[, y, drop = FALSE]) + 1
    )
    data <- pat$pattern
    colnames(data) <- y
  }
  ...
  class(mu) <- "matrix"
  thres <- matrix(0, nrow = length(y), max(1, attributes(ord)$K - 1))
  rownames(thres) <- y
  ...
  if (
    !inherits(yl[1, 1], c("numeric", "integer", "logical")) ||
...
normal_hessian.lvm <- function(x, p, outer = FALSE, data2 = NULL, ...) {
  ...
  if (!outer) {
    f <- function(p) {
      ...
      do.call(
        "normal_objective.lvm",
        c(list(x, p = p, indiv = FALSE, data2 = data2), dots)
      )
    }
    g <- function(p) {
      set.seed(1)
