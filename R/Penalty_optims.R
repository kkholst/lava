ISTA <- function(start, objective, gradient, hessian,...){
  ...
  proxL2 <- function(x, lambda, test.penalty){
    if(test.penalty){
      1 / (1 + lambda*t_k) * x
    }else{
      x
  ...
  t_k <- 1/max(abs(eigen(hessian(x_k, penalty = NULL))$value)) # may be computationnaly expensive 
  ...
  ## requested operator
  if(all(lambda1 == 0) && all(lambda2 == 0)){
    proxOperator <- function(x, lambda1, lambda2){x}
  }else if(all(lambda2 == 0)){
    test.penalty <- lambda1>0
    
    proxOperator <- function(x, lambda1, lambda2){
      mapply(proxL1, x = x, lambd = lambda1, test.penalty = test.penalty)
    }
  }else if(all(lambda1 == 0)){
    test.penalty <- lambda2>0
   
     proxOperator <- function(x, lambda1, lambda2){
      mapply(proxL2, x = x, lambda = lambda2, test.penalty = test.penalty)
    }
  }else{
    test.penalty1 <- lambda1 > 0
    test.penalty2 <- lambda2 > 0
    
    proxOperator <- function(x, lambda1, lambda2){
      mapply(proxL2, 
             x = mapply(proxL1, x = x, lambd = lambda1, test.penalty = test.penalty1),
             lambda = lambda2, test.penalty = test.penalty2)
    }
  }
  ...
  if(("FAST" %in% names(control) == FALSE) || (control$FAST == FALSE)){
    ...
    while(test.cv == FALSE && iter <= control$iter.max){
      ...
      x_k <- proxOperator(x = x_km1 - t_k * gradient(x_km1, penalty = NULL), 
    ...
    while(test.cv==FALSE && iter <= control$iter.max){
      ...
      x_k <- proxOperator(x = y_k - t_k * gradient(y_k, penalty = NULL), 
                          lambda1 = lambda1, lambda2 = lambda2)
