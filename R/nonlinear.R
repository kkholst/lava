...
naturalcubicspline <- function(
  x,
  knots = stats::median(x, na.rm = TRUE),
...
ncspred <- function(mu, var, knots = c(-5, 0, 5)) {
  ...
  g <- function(x, tau) {
    ...
    k * (2 * v + x2) * exp(-(x0 / (sqrt(2 * v)))^2) + x0 * (x2 + 3 * v) * p0
  }
  n <- NROW(mu)
...
nonlinear.lvm <- function(
  ...
  if (length(to) > 1) {
    stop("Supply only one response variable")
  }
  if (length(from) > 1) {
    stop("Supply only one explanatory variable")
  }
  ...
  if (
    tolower(type)[1] %in%
      c("ncs", "spline", "naturalspline", "cubicspline", "natural cubic spline")
  ) {
    if (is.null(knots)) {
      stop("Need cut-points ('knots')")
    }
    if (length(knots) < 3) {
    ...
    if (missing(names)) {
      names <- paste0(from, "_", seq(length(knots) - 1))
    }
    f <- function(p, x) {
      B <- cbind(
        1,
  ...
  if (tolower(type)[1] == "quadratic") {
    if (missing(names)) {
      names <- paste0(from, "_", 1:2)
    }
    f <- function(p, x) p[1] + p[2] * x + p[3] * (x * x)
    pred <- function(mu, var, ...) {
      structure(
        cbind(mu[, 1], mu[, 1]^2 + var[1]),
  ...
  if (tolower(type)[1] %in% c("exp0")) {
    if (missing(names)) {
      names <- paste0(from, "_", 1)
    }
    f <- function(p, x) p[1] + p[2] * exp(x)
    pred <- function(mu, var, ...) {
      structure(
        cbind(exp(0.5 * var[1] + mu[, 1])),
        dimnames = list(NULL, names)
      )
    }
  }

  object$attributes$nonlinear[[to]] <- list(
    x = from,
    p = length(names) + 1,
    newx = names,
    f = f,
