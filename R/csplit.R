...
csplit <- function(
  ...
  if (is.null(p)) {
    ...
    if (p < 1) {
      ## two folds (size N*p and N*(1-p))

      idx1 <- base::sample(N, base::round(p * N), replace = replace)
      idx <- list(idx1, base::sample(setdiff(seq(N), idx1), replace = replace))
    } else {
      ## Number of folds (equal size)
...
foldr <- function(n, K = 5, rep = 1, list = TRUE) {
  res <- replicate(
    rep,
    split(sample(seq(n)), rep(seq(K), length = n)),
    simplify = FALSE
  )
  if (!list) {
    ids <- rep(seq_len(length(res[[1]])), unlist(lapply(res[[1]], length)))
