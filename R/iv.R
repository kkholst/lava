  ...
  for (v in vv) {
    ...
    if (sum(pred)>0) {
      ...
      if (length(pred.lat)>0) {
        for (i in 1:length(pred.lat)) {
          uncor <- which(colSums(VV[ecor,k+1:k,drop=FALSE])==0)
      ...
      covariates <- c(setdiff(colnames(A)[A[v,]==1],latent(m)),allpred)
