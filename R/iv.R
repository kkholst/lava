...
IV <- function(m, data, R2thres = 0, type = "robust", ...) {
  if (length(constrain(m)) > 0) {
    stop("Nonlinear constrains not supported!")
  }
  ...
  if (length(lat.idx) == 0) {
    stop("Estimator only defined for models with latent variable")
  }
  y.var <- endogenous(m)
  ...
  if (any(unlist(lapply(y.scale, function(x) length(x))) < 1)) {
    stop("At least one scale-measurement pr. latent variable")
  }
  ...
  for (e in lat.idx) {
    ...
    if (!(y0 %in% lat.idx)) {
      ...
      while (i < length(y.indicators)) {
        ...
        if (!is.null(pp)) {
          Ecor <- c(Ecor, list(which(vars(m) %in% pp[-1])))
  ...
  for (v in vv) {
    ...
    if (sum(pred) > 0) {
      ...
      covariates <- unique(c(
        setdiff(colnames(A)[A[v, ] == 1], latent(m)),
        intpred
      ))
      ...
      parname <- c(
        parname,
        c(vars(m)[v0], paste(vars(m)[v], XX, sep = lava.options()$symbol[1]))
      )
    } else {
  ...
  for (i in seq_along(Z.)) {
    Zhat <- c(Zhat, list(V.[[i]] %*% (P0[[i]] %*% Z.[[i]])))
  }
  ZhatLS <- lapply(Zhat, LS)
  theta <- list()
  for (i in seq_along(Y.)) {
    theta <- c(theta, list(ZhatLS[[i]] %*% Y.[[i]]))
  }
  u <- c()
  for (i in seq_along(Y.)) {
  ...
  for (i in seq_along(theta)) {
    for (j in seq(i, length(theta))) {
      ...
      if (type == "robust") {
        zi <- ZhatLS[[i]]
        for (k in seq(nrow(zi))) {
          zi[k, ] <- zi[k, ] * u[, i]
        }
        zj <- ZhatLS[[j]]
        for (k in seq(nrow(zj))) {
          zj[k, ] <- zj[k, ] * u[, j]
        }
        uZZ <- zi %*% t(zj)
      } else {
  ...
  parname[which(parname %in% eta.surrogate)] <- names(eta.surrogate)[which(
    eta.surrogate %in% parname
  )]

  coef <- cbind(unlist(theta), diag(vartheta)^0.5)
  rownames(coef) <- parname
  colnames(coef) <- c("Estimate", "Std.Err")
