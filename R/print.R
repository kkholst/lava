    ...
    if (is.null(res)) {
      ...
      for (f in ff) {
        ...
        y <- trim(y)
        {
          ...
          if (
            is.list(distribution(x)[[y]]) &&
              is.vector(distribution(x)[[y]][[1]])
          ) {
            col2 <- "fixed"
          }
          if (L[y]) {
            col2 <- paste0(col2, ", latent")
          }
          if (y %in% exo) {
...
`print.lvmfit` <-
  function(x, type = 2, labels = FALSE, ...) {
    print(
      CoefMat(x, labels = labels, type = type, ...),
      quote = FALSE,
      right = TRUE
    )
    ...
    if (!is.null(pseudo) && pseudo) {
      warning("Singular covariance matrix. Pseudo-inverse used.")
    }
    invisible(x)
  }
...
print.multigroupfit <- function(x, groups = NULL, ...) {
  if (is.null(groups)) {
    if (x$model$missing) {
      ...
      groups <- unlist(lapply(orggroup, function(i) {
        which.min(nmis[which(modelclass == i)]) + cumsumgroup[i]
      })) ##  groups with max. number of variables
      for (i in seq_len(length(groups))) {
        if (nmis[groups[i]] > 0) {
          warning(
            "No complete cases in group ",
            i,
            ". Showing results of group with max number of variables. All coefficients can be extracted with 'coef'. All missing pattern groups belonging to this sub-model can be extracted by calling: coef(..., groups=c(",
            paste(which(modelclass == i), collapse = ","),
            "))"
          )
        }
      }
  ...
  for (cc in res) {
    ...
    if (!x$model$missing) {
      cat(" (n=", nrow(Model(x)$data[[groups[counter]]]), ")", sep = "")
    }
    cat("\n")
...
printmany <- function(
  ...
  if (!is.null(name2)) {
    oldname <- colnames(res)
    res <- cbind(res, rep("", nrow(res)))
    colnames(res) <- c(oldname, name2)
  }
  ...
  if (print) {
    print(res, quote = FALSE, right = right, ...)
  }
  invisible(res)
}
