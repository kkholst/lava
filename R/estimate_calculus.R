...
merge.estimate <- function(
  ...
  if (sum(!hasIC) > 0L) {
    # Some objects do not have influence function
  ...
  if (!missing(id) && is.null(id)) {
    ## Independence between datasets in x,y,...
    nn <- unlist(lapply(
      objects,
  ...
  if (missing(id)) {
    if (paired) {
      ## One-to-one dependence between observations in x,y,...
...
log.estimate <- function(x, base = exp(1), ...) {
  estimate(
    ...
    function(p) {
      y <- log(p, base = base)
      structure(y, grad = diag(1 / (p * log(base)), nrow = length(p)))
    },
    ...
  )
}
...
sum.estimate <- function(x, ...) {
  estimate(
    x,
    function(p) {
      structure(sum(p), grad = matrix(1, nrow = 1, ncol = length(p)))
    },
    ...
  )
}
...
"+.estimate" <- function(e1, e2, ...) {
  operator_estimate(
    ...
    function(x, y, x_const = FALSE, y_const = FALSE) {
      structure(
        x + y,
        grad = operator_grad(x, y, x_const, y_const, dx = 1, dy = 1)
      )
    },
    ...
  )
...
"*.estimate" <- function(e1, e2, ...) {
  operator_estimate(
    ...
    function(x, y, x_const = FALSE, y_const = FALSE) {
      structure(
        x * y,
        grad = operator_grad(x, y, x_const, y_const, dx = y, dy = x)
      )
    },
    ...
  )
...
"/.estimate" <- function(e1, e2, ...) {
  operator_estimate(
    ...
    function(x, y, x_const = FALSE, y_const = FALSE) {
      structure(
        x / y,
        grad = operator_grad(x, y, x_const, y_const, dx = 1 / y, dy = -x / y^2)
      )
    },
    ...
  )
...
"==.estimate" <- function(e1, e2) {
  if (!(is.numeric(e1) || is.numeric(e2))) {
    stop("numeric comparator needed")
  }
