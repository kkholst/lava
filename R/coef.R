...
`coef.lvm` <-
  function(
    ...
    if (!missing(p)) {
      coefs <- matrix(NA, nrow = length(p), ncol = 4)
      coefs[, 1] <- p
      rownames(coefs) <- c(
        coef(object, mean = TRUE, fix = FALSE)[c(seq_len(
          index(object)$npar.mean
        ))],
        {
          if (index(object)$npar > 0) paste0("p", seq_len(index(object)$npar))
        },
        {
          if (index(object)$npar.ex > 0) {
    ...
    A <- AP$A
    A[index(object)$M1 == 0] <- "0" ## Only free parameters
    P <- AP$P
    P[index(object)$P1 == 0] <- "0"
    P[upper.tri(P)] <- "0"
    nn <- vars(object)
...
`coef.lvmfit` <-
  function(
    ...
    if (type < 0 && !is.null(names(res))) {
      return(res)
    }

    if (is.null(object$control$meanstructure)) {
      meanstructure <- TRUE
    } else {
      meanstructure <- object$control$meanstructure
    }
    npar <- index(object)$npar
    npar.mean <- index(object)$npar.mean * meanstructure
    ...
    if (inherits(object, "lvm.missing")) {
      if (length(object$cc) == 0) {
        ...
        myorder <- na.omit(modelPar(
          object$multigroup,
          seq_len(npar + npar.mean)
        )$p[[object$cc]])
        myorder.reg <- na.omit(modelPar(object$multigroup, seq_len(npar))$p[[
          object$cc
        ]])
    ...
    if (type < 0) {
      names(res)[seq_len(length(myorder))] <- coef(
        Model(object),
        fix = FALSE,
        mean = meanstructure,
        symbol = symbol
    ...
    if (!is.null(std)) {
      stdCoef <- stdcoef(object)
      {
        switch(
          ...
          xy = {
            Astd <- stdCoef$AstarXY
            Pstd <- stdCoef$PstarXY
            vstd <- stdCoef$vstarXY
          },
          yx = {
            Astd <- stdCoef$AstarXY
            Pstd <- stdCoef$PstarXY
    ...
    nlincon <- matrix(
      Model(object)$par %in% names(constrain(Model(object))),
      nrow(A)
    )
    if (missing(data)) {
      data <- matrix(0, ncol = length(index(Model(object))$manifest))
      colnames(data) <- index(Model(object))$manifest
    }
      ...
      for (j in seq_len(nrow(A))) {
        ...
        if (val != "0") {
          ...
          if (!is.na(matching)) {
            ...
            if (nlincon[j, i]) {
              newrow <- matrix(
                nlincon.estimates[Model(object)$par[j, i], ],
                nrow = 1
              )
            } else {
    ...
    nlincon.var <- matrix(
      Model(object)$covpar %in% names(constrain(Model(object))),
      nrow(P)
    )
      ...
      for (i in seq_len(ncol(p$P))) {
        for (j in seq(i, nrow(p$P))) {
          ...
          if (!(i %in% para.idx)) {
            if (
              val != "0" &
                !any(vars(object)[c(i, j)] %in% index(Model(object))$exogenous)
            ) {
              if (
                type > 1 |
                  !all(vars(object)[c(i, j)] %in% index(Model(object))$manifest)
              ) {
                matching <- match(val, rownames(coefs))
                matched <- c(matched, matching)
                ...
                if (!is.na(matching)) {
                  ...
                  if (nlincon.var[j, i]) {
                    newrow <- matrix(
                      nlincon.estimates[Model(object)$covpar[j, i], ],
                      nrow = 1
                    )
                  } else {
                    newrow <- matrix(
                      c(Model(object)$covfix[j, i], NA, NA, NA),
                      nrow = 1
                    )
                  }
                }
    ...
    ## Mean parameter:
    nlincon.mean <- lapply(Model(object)$mean, function(x) {
      x %in% names(constrain(Model(object)))
    })
      ...
      for (i in seq_len(length(meanpar))) {
        if (!index(Model(object))$vars[i] %in% index(Model(object))$exogenous) {
          ...
          if (!is.na(matching)) {
            ...
            if (nlincon.mean[[i]]) {
              newrow <- matrix(
                nlincon.estimates[Model(object)$mean[[i]], ],
                nrow = 1
              )
            } else {
          ...
          if (
            labels &
              !(is.na(intfix(Model(object))[[i]]) |
                is.numeric(intfix(Model(object))[[i]]))
          ) {
          ...
          if ((index(object)$v1[i] == 1) | type > 2) {
            ...
            Type <- c(
              Type,
              ifelse(!(i %in% para.idx), "intercept", "parameter")
            )
    ...
    if (!is.null(std)) {
      mycolnames <- c(mycolnames, paste("std", std, sep = "."))
    }
...
##' @export
CoefMat.multigroupfit <- function(
  x,
  type = 9,
  labels = lava.options()$coef.names,
  symbol = lava.options()$symbol[1],
  data = NULL,
  groups = seq(Model(x)$ngroup),
  ...
) {
  cc <- coef(
    x,
  ...
  for (i in groups) {
    ...
    Vcov <- vcov(x)[parpos[[k]], parpos[[k]], drop = FALSE]
    colnames(Vcov) <- rownames(Vcov) <- rownames(mycoef)
    m0$vcov <- Vcov
      ...
      if (count == 0) {
        ...
        for (j in seq_len(NROW(newnlin))) {
          if (!(rownames(newnlin)[j] %in% nlincon.names)) {
            nlincon.estimates <- rbind(
              nlincon.estimates,
              newnlin[j, , drop = FALSE]
            )
...
CoefMat <- function(
  ...
  if (Nreg > 0) {
    ...
    latent.from <- latent.from[which(is.na(match(
      attributes(cc)$var[latent.from],
      latent.var
    )))]
    ...
    if (Nmeas > 0) {
      ...
      for (i in latent.var) {
        ...
        Delta <- !Delta
        Myidx <- which(
          attributes(cc)$from == i &
            attributes(cc)$type == "regression" &
            !(attributes(cc)$var %in% latent.var)
        )
  ...
  if (Nvar > 0) {
    ...
    } else if (!is.null(x$opt$dispname)) {
...
stdcoef <- function(x, p = coef(x), ...) {
  ...
  colnames(N) <- rownames(N) <- vars(x)
  iN <- N
  diag(iN)[diag(N) > 0] <- 1 / diag(iN)[diag(N) > 0]
  diag(iN)[diag(N) == 0] <- NA
  Nn <- N
  Nn[] <- 0
  diag(Nn) <- 1
  ...
  res <- list(
    par = cbind(p, c(pstd, rep(NA, k)), c(pstdXY, rep(NA, k))),
    AstarXY = AstarXY,
    PstarXY = PstarXY,
    vstarXY = vstarXY,
    AstarY = AstarY,
