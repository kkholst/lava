...
score.lvm <- function(
  ...
  if (length(xfix) > 0 | length(xconstrain) > 0) {
    ...
    if (length(score) < length(p)) {
      score <- c(score, rep(0, length(p) - length(score)))
    }
    return(score)
  }
  cl$constrain <- FALSE
  ...
  if (NCOL(score) < length(p)) {
    score <- cbind(rbind(score), rep(0, length(p) - NCOL(score)))
  }
