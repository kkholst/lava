...
colorbar <- function(
  ...
  if (missing(label.offset)) {
    label.offset <- barsize / 3
  }
  delta <- ifelse(X, x.range[1], y.range[1])
  if (!is.null(values)) {
    dM <- diff(range(values)) / (nlut - 1)
  }
  ...
  if (!is.null(values)) {
    for (i in seq_len(nlut + 1) - 1) {
      pos <- delta + (i - 1) * scale
      rund <- format(
        round(min(values) + dM * i, max(1, digits)),
        digits = digits
      )
      x0 <- pos + (1 + 0.5) * scale
      y0 <- y.range[2] + label.offset
      if (!X) {
        y0 <- x0
        if (position == 1) {
          x0 <- x.range[1] - label.offset
        }
        if (position == 2) {
          x0 <- x.range[1] + label.offset * 5
        }
