missingModel <- function(
  ...
  if (!inherits(model, "lvm")) {
    stop("Needs a lvm-object")
  }
  if (type == 3) {
    var <- manifest(model)
  }
  ...
  mis.type <- apply(data.mis, 1, function(x) {
    which(apply(patterns, 1, function(y) identical(x, y)))
  })
  ...
  for (i in setdiff(seq_len(nrow(patterns)), pattern.allmis)) {
    ...
    if (length(xmis) > 0) {
      misx <- ex0[apply(d0[xmis, ex0, drop = FALSE], 2, function(x) {
        any(is.na(x))
      })]
      if (!warned) {
        warning(
...
estimate.MAR <- function(
  ...
  if (K > 0) {
    ...
    cov0 <- cov(datax, use = "pairwise.complete.obs") *
      (nrow(datax) - 1) /
      nrow(datax)
  ...
  if (!is.null(names(control$start))) {
    ...
    names(start0)[which(!is.na(paridx))] <- names(control$start[na.omit(
      paridx
    )])
  ...
  if (onlymodel) {
    return(list(
      ...
      weights = val$weights,
      data2 = val$data2,
      cluster = val$clusters
    ))
  }

  if (all(unlist(lapply(val$weights, is.null)))) {
    val$weights <- NULL
  }
  if (all(unlist(lapply(val$data2, is.null)))) {
    val$data2 <- NULL
  }
  if (all(unlist(lapply(val$clusters, is.null)))) {
    val$clusters <- NULL
  }
  ...
  if (length(nmis) > 0 & length(mycc) > 0) {
    ## Any individuals with all missing?
  ...
  if (length(xfix) > 0) {
    ...
    for (i in seq_along(xfix)) {
      regfix(x, from = vars(x)[rowpos[[i]]], to = vars(x)[colpos[[i]]]) <-
        rep(
          colMeans(data[, xfix[i], drop = FALSE], na.rm = TRUE),
          length(rowpos[[i]])
        )
    }
