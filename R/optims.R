...
estimatingfunction <- function(
  ...
  if (!missing(hessian) && !is.null(hessian)) {
    ...
    hessian <- function(x) {
      numDeriv::jacobian(gradient, x, method = lava.options()$Dmethod)
    }
    mygrad <- function(x) {
      H <- hessian(x)
      S <- gradient(x)
...
estimatingfunction0 <- function(..., hessian = NULL) {
  estimatingfunction(..., hessian = hessian)
}
...
NR <- function(
  ...
  if (!is.logical(control0$backtrack) || length(control0$backtrack) != 1) {
    ...
    if (
      any(!is.numeric(control0$backtrack)) || any(abs(control0$backtrack) > 1)
    ) {
  ...
  oneiter <- function(p.orig, Dprev, return.mat = FALSE, iter = 1) {
    ...
    if (!is.logical(control0$backtrack) && !is.null(obj)) {
      # Back-tracking based on objective function evaluations
    ...
    Delta <- control0$stepsize *
      tryCatch(solve(I, cbind(as.vector(D))), error = function(...) {
        ## Fall back to Pseudo-Inverse using SVD:
    ...
    if (identical(control0$backtrack, TRUE)) {
      ...
      # objective(p.orig) - obj(p) <= mu*Lambda*grad(p.orig)*Delta
      ## curvature
      c_D.origin_Delta <- control0$backtrack * c(rbind(D) %*% Delta)
      p <- p.orig + as.vector(Lambda * Delta)

      mD0 <- c(
        objective.origin + Lambda * c_D.origin_Delta[1],
        abs(c_D.origin_Delta[2])
      ) #
      objective.new <- obj(p)
