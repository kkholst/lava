...
`estimate.multigroup` <- function(
  ...
  if (!is.null(Optim$start) & length(Optim$start) == (x$npar + x$npar.mean)) {
    ...
    if (!is.null(Optim$start)) {
      ...
      if (any(!is.na(ppos))) {
        mystart[ppos] <- Optim$start[na.omit(match(
  ...
  for (i in seq_len(x$ngroup)) {
    vpos <- sapply(
      x$parlist[[i]][variances(x$lvm[[i]], mean = FALSE)],
      function(y) char2num(substr(y, 2, nchar(y)))
    )
    if (length(vpos) > 0) {
  ...
  if (!missing(weights)) {
    if (is.character(weights)) {
      ...
      for (i in seq_along(x$data)) {
        ...
        colnames(newweights) <- index(x$lvm[[i]])$endogenous[seq_len(ncol(
          newweights
        ))]
  ...
  if (!missing(data2)) {
    if (is.character(data2)) {
      ...
      for (i in seq_along(x$data)) {
        ...
        colnames(newdata) <- index(x$lvm[[i]])$endogenous[seq_len(ncol(
          newdata
        ))]
  ...
  for (f in myhooks) {
    for (i in seq_len(x$ngroup)) {
      ...
      if (!is.null(res$weights)) {
        newweights <- c(newweights, list(res$weights))
      }
      if (!is.null(res$data2)) {
        newdata2 <- c(newdata2, list(res$data2))
      }
  ...
  for (i in seq_len(x$ngroup)) {
    ...
    xconstrain0 <- intersect(
      unlist(lapply(constrain(x0), function(z) attributes(z)$args)),
      manifest(x0)
    )
  ...
  if (Xconstrain) {
    for (i in seq_len(x$ngroup)) {
      ...
      if (length(xconstrain0) > 0) {
        ...
        if (
          xconstrainM &
            ((is.null(control$method) || Optim$method == "nlminb0") &
              (lava.options()$test & estimator == "gaussian"))
        ) {
          XconstrStdOpt <- FALSE
  ...
  if (Xfix | (Xconstrain & XconstrStdOpt | !lava.options()$test)) {
      ...
      for (k in seq_len(x$ngroup)) {
        ...
        for (i in seq_along(myfix0$var)) {
          for (j in seq_along(myfix0$col[[i]])) {
            regfix(
              x0,
              from = vars(x0)[myfix0$row[[i]][j]],
              to = vars(x0)[myfix0$col[[i]][j]]
            ) <-
        ...
        parkeep <- c(
          parkeep,
          parord[[k]][
            coef(x1, mean = TRUE, fix = FALSE) %in%
    ...
    myGrad <- function(theta) {
      ...
      for (k in seq_len(x$ngroup)) {
        ...
        D <- D0
        D[parord[[k]]] <- rowSums(sapply(seq_len(nrow(mydata[[k]])), myfun))
        res <- res + D
      }
      if (Optim$constrain) {
    ...
    myInformation <- function(theta) {
      ...
      for (k in seq_len(x$ngroup)) {
        ...
        myfun <- function(ii) {
          if (Xfix) {
            for (i in seq_along(myfix0$var)) {
              x0$fix[cbind(myfix0$row[[i]], myfix0$col[[i]])] <- index(
                x0
              )$A[cbind(myfix0$row[[i]], myfix0$col[[i]])] <-
                x$data[[k]][ii, xfix[[k]][i]]
            }
          }
          I <- I0
        ...
        if (!is.null(attributes(L[[1]])$grad)) {
          grad <- grad +
            rowSums(matrix(
              (unlist(lapply(L, function(x) attributes(x)$grad))),
              ncol = length(L)
            ))
        }
        res <- res +
          apply(
            array(
              unlist(L),
              dim = c(length(theta), length(theta), nrow(x$data[[k]]))
            ),
            c(1, 2),
            sum
          )
    ...
    for (j in seq_len(x$ngroup)) {
      ...
      for (i in seq_len(length(constrain(x0)))) {
        ...
        if (length(xx) > 0) {
          ...
          y <- names(which(unlist(lapply(intercept(x0), function(x) {
            x == parname
          }))))
    ...
    MkOffset <- function(pp, x, data, xconstrain, grad = FALSE) {
      if (length(xconstrain) > 0) {
        ...
        for (i in seq_len(length(xconstrain))) {
          ...
          mu <- with(
            xconstrain[[i]],
            apply(data[, exo, drop = FALSE], 1, function(x) {
              func(
                unlist(c(pp, x))[myidx]
              )
            })
    ...
    myInformation <- function(theta) {
      ...
      D <- myGrad(theta0)
      if (Optim$constrain) {
        res[constrained, -constrained] <- apply(
          res[constrained, -constrained, drop = FALSE],
          2,
          function(x) x * theta[constrained]
        )
        res[-constrained, constrained] <- t(res[constrained, -constrained])
        if (sum(constrained) == 1) {
          res[constrained, constrained] <- res[constrained, constrained] *
            outer(theta[constrained], theta[constrained]) -
            (D[constrained])
        } else {
          res[constrained, constrained] <- res[constrained, constrained] *
            outer(theta[constrained], theta[constrained]) -
            diag(D[constrained], nrow = length(constrained))
        }
      }
  ...
  if (!exists(InformationFun)) {
    myInformation <- NULL
  } else if (is.null(get(InformationFun))) {
    myInformation <- NULL
  }
  if (is.null(get(GradFun))) {
    myGrad <- NULL
  }
  ...
  if (!XconstrStdOpt) {
    ...
    if (is.null(myInformation)) {
      ...
      if (!is.null(myGrad) & XconstrStdOpt) {
        myInformation <- function(theta) {
          numDeriv::jacobian(myGrad, theta, method = lava.options()$Dmethod)
        }
      } else {
  ...
  asVar <- tryCatch(Inverse(I), error = function(e) {
    matrix(NA, length(mystart), length(mystart))
  })
...
estimate_lvmlist <-
  function(
    ...
    if (is.data.frame(data)) {
      warning(
        "Only one dataset - going for standard analysis on each submodel."
      )
      res <- c()
      for (i in seq_len(nm)) {
        res <- c(
    ...
    if (nm != length(data)) {
      stop("Supply dataset for each model")
    }
    ...
    for (i in seq_along(x)) {
      data0 <- data[[i]]
      xfix0 <- colnames(data0)[
        (colnames(data0) %in% parlabels(x[[i]], exo = TRUE))
      ]
      xfix <- c(xfix, list(xfix0))
      if (length(xfix0) > 0) {
        ## Yes, random slopes
