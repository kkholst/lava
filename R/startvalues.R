...
##' @export
starter.multigroup <- function(
  x,
  starterfun = startvalues2,
  meanstructure = TRUE,
  messages = 0,
  ...
) {
  ...
  for (i in seq_len(x$ngroup)) {
    ...
    if (nrow(mydata) < 3) {
      ...
      s0 <- do.call(
        starterfun,
        list(x$lvm[[i]], S = S, mu = mu, messages = messages)
      )
    }
    s <- c(s, list(s0))
  }
  ...
  pg <- vector("list", x$npar)
  for (i in seq_len(length(pg))) {
    pg[[i]] <- rep(0, x$ngroup)
  }
  meang <- vector("list", x$npar.mean)
  for (i in seq_len(length(meang))) {
    meang[[i]] <- rep(0, x$ngroup)
  }
...
##' @export
`startvalues2` <- function(x, S, mu = NULL, debug = FALSE, messages = 0, ...) {
  if (messages > 0) {
  ...
  e <- lapply(g, function(y) {
    estimate(
      y,
      data = list(
        S = S[manifest(y), manifest(y), drop = FALSE],
        mu = mu[manifest(y)],
        n = 100
      ),
      control = list(
...
startvalues <- ## McDonald & Hartmann, 1992
  function(
    ...
    S <- reorderdata.lvm(x, S)
    if (nrow(S) != length(manifest(x))) {
      stop("Number of observed variables in data and models does not agree.")
    }
        ...
        C[i, j] <- C[j, i] <-
          if (i == j) {
            ...
            if ((any(inRelation.i)) | (any(inRelation.j))) {
              ...
              denominator <- sqrt(
                sum(R[which(inRelation.i), which(inRelation.i)]) *
    ...
    for (j in seq_len(m)) {
      ...
      Ahat[j, relation] <- tryCatch(
        Inverse(
          C[relation, relation] + diag(nrow = sum(relation)) * delta,
          tol = 1e-3
        ) %*%
          C[relation, j],
        error = function(...) 0
