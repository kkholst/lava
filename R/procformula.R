...
procformula <- function(
  ...
  if (xidx == 2) {
    if (length(grep("[a-zA-Z0-9_]*\\(.*\\)$", yx[[xidx]])) > 0) {
      ## rhs of the form F(x+y)
      invlink <- strsplit(yx[[xidx]], "\\(.*\\)")[[1]][1]
      if (invlink %in% c("f", "v", "I", "") || grepl("\\+", invlink)) {
  ...
  xxf <- lapply(as.list(xx), function(x) {
    decomp.specials(x, NULL, pattern2 = "\\[|~", perl = TRUE)
  })
  xs <- unlist(lapply(xxf, function(x) x[1]))

  ## Alter intercepts?
  intpos <- which(
    vapply(xs, function(x) grepl("^[\\+\\-]*[\\.|0-9]+$", x), 0) == 1
  )
  ## Match '(int)'
  intpos0 <- which(
    vapply(X, function(x) grepl("^\\([\\+\\-]*[\\.|0-9]+\\)$", x), 0) == 1
  )

  yy <- ys <- NULL
  if (length(lhs) > 0) {
    yy <- decomp.specials(lhs)
    yyf <- lapply(yy, function(y) {
      decomp.specials(y, NULL, pattern2 = "[", fixed = TRUE, perl = FALSE)
    })
    ...
    if (length(lhs) > 0) {
      ...
      if (!is.null(invlink)) {
        if (invlink == "") {
          ...
          ff <- function(x) {}
          body(ff) <- parse(text = paste0(invlink, "(x)"))
    ...
    for (i in seq_len(length(xs))) {
      ...
      if (length(xf0) > 1) {
        ...
        transform(object, y = xf0, x = xtr) <- function(x) {
          structure(
            model.matrix(ftr, as.data.frame(x)),
            dimnames = list(NULL, xf0)
          )
        }
      }
