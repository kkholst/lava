...
sim.default <- function(
  ...
  if (is.null(x)) {
    stop("Must give new function argument 'f'.")
  }
  ...
  robx <- function(iter__, ...) {
    ...
    if (!is.null(extra)) {
      # append extra par. not part of the estimate object
      res <- c(res, extra)
    }
    return(structure(
      res,
...
Print <- function(x, n = 5, digits = max(3, getOption("digits") - 3), ...) {
  ...
  if (n < 1) {
    ...
    if (NROW(x) <= (2 * n)) {
      ...
      print(
        rbind(base::as.matrix(hd), sep, base::as.matrix(tl)),
        quote = FALSE,
        ...
      )
    }
...
#' @export
print.summary.sim <- function(
  x,
  group = list(
    c("^mean$", "^sd$", "^se$", "^se/sd$", "^coverage", "^length"),
    c("^min$", "^[0-9.]+%$", "^max$"),
    c("^na$", "^missing$"),
    c("^true$", "^bias$", "^rmse$")
  ),
  ...
  gg <- lapply(group, function(x) unlist(lapply(x, function(v) grep(v, nn))))
...
summary.sim <- function(
  ...
  if (missing(confint)) {
    ...
    if (
      ...
      pi_est <- par.index[["estimate"]]
      if (is.null(lo)) {
        lo <- rep(-Inf, length(pi_est))
      }
      if (is.null(up)) {
        up <- rep(Inf, length(pi_est))
      }
      confint <- list()
      for (i in seq_len(np)) {
        j <- match(estimate[i], pi_est)
  ...
  if (minimal) {
    fun <- function(x, se, confint, ...) {
      res <- c(Mean = mean(x, na.rm = TRUE), SD = sd(x, na.rm = TRUE))
  ...
  mfun <- function(x, ...) {
    ...
    names(res) <- c("Mean", "SD", if (length(quantiles) > 0) nq, "Missing")
    res
  }
  tm <- attr(object, "time")
  ...
  if (is.list(confint)) {
    confint <- unlist(confint)
  }
  if (!missing(fun)) {
    if (!is.null(estimate)) {
      m.est <- object[, estimate, drop = FALSE]
    } else {
      m.est <- object
    }
    m.se <- NULL
    if (!is.null(se)) {
      m.se <- object[, se, drop = FALSE]
    }
    m.ci <- NULL
    if (!is.null(confint)) {
      m.ci <- object[, confint, drop = FALSE]
    }
    res <- lapply(
      seq(ncol(m.est)),
      function(i, ...) {
  ...
  if (!is.null(se)) {
    ...
    if (length(se) != ncol(est)) {
      stop("'se' should be of same length as 'estimate'.")
    }
    est <- rbind(
      est,
      SE = apply(object[, se, drop = FALSE], 2, function(x) {
        val <- c(mean(x, na.rm = TRUE))
      })
    )
  ...
  if (!is.null(confint) && (length(confint) > 1 || confint)) {
    ...
    if (length(confint) == 1 && confint) {
      if (is.null(se)) {
        stop("Supply confidence limits or SE")
      }
      confint <- c()
      pos <- ncol(object)
    ...
    if (length(confint) != 2 * length(estimate)) {
      stop("'confint' should be of length 2*length(estimate).")
    }
    Coverage <- c()
    Length <- c()
    for (i in seq_along(estimate)) {
