...
multinomial <- function(
  ...
  if (!is.table(x)) {
    if (NCOL(x) != 2L) {
      stop("Matrix or data.frame with one or two columns expected")
    }
    x <- as.data.frame(x)
    x[, 1] <- as.factor(x[, 1])
    x[, 2] <- as.factor(x[, 2])
  ...
  coefs <- as.vector(P)
  names(coefs) <- as.vector(outer(seq(k1), seq(k2), function(...) {
    paste0("p", ...)
  }))
  ...
  res <- list(
    ...
    position = Pos,
    call = match.call(),
    levels = list(lev1, lev2),
    data = x,
    position1 = position1,
    position2 = position2, ## Position of marginals)
    terms = trm
  )
  class(res) <- "multinomial"
  if (length(list(...)) > 0) {
...
predict.multinomial <- function(object, newdata, type = c("prob", "map"), ...) {
  ...
  pcond <- object$P
  for (i in seq(nrow(pcond))) {
    pcond[i, ] <- pcond[i, ] / px[i]
  }
