LassoPath_lvm <- function(beta0, hessianLv, gradientLv, objectiveLv,
                          indexPenalty, indexNuisance,
                          gradientPen = NULL, iter_max = length(beta0)*10,  scalePen = NULL,
                          dataY, dataX){
  
  res <- orthoData(dataX = dataX)
  dataX <- res$data
...
nextNode_lvm <- function(hessianLv, gradientLv, gradientPen,
  ...
  hess_Lv <- hessianLv(beta) * (scalePen %o% scalePen)
  ...
  ## find next knot
  gamma1 <- -beta[set_A]/(grad_B.A*lambda1[set_A]*scalePen[set_A]) 
...
orthoData <- function(data, formula, dataX = NULL){
  ...
  dataX <- dataX - cbind(ones) %*% solve(crossprod(ones), crossprod(ones, dataX))
