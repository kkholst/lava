...
# tweaked to remove top level sections (#) from TOC
render_toc <- function(
  filename,
  ...
  x <- sapply(x, function(h) {
    ...
    reps <- if (level > 0) (level - 1) * 4 else 0 # tweak to remove # sections
    paste0(strrep(" ", reps), "- [", header_text, "](#", header_slug, ")")
  })
  x <- x[x != ""]

  x <- x[-1] # tweak to remove # sections
  nn <- unname(sapply(names(x), \(h) substring(h, 2)))
  names(x) <- nn
...
Assessing goodness-of-fit, here the linearity between u2 and u1 (requires the `gof` package)
