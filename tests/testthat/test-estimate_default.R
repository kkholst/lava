...
test_that("estimate.default misc", {
  ...
  testthat::expect_true(
    rownames(
      estimate(l1, function(x) list("X" = x[2], "Z" = x[3]), keep = "X")$coefmat
    ) ==
      "X"
  )
  testthat::expect_true(
  ...
  a0 <- estimate(
    l1,
    function(p, data) p[1] + p[2] * data[, "x"],
    average = TRUE
  )
  a1 <- estimate(
    l1,
    function(p, data) p[1] + p[2] * data[, "x"] + p[3],
    average = TRUE
  )
  a <- merge(a0, a1, labels = c("a0", "a1"))
  estimate(a, diff)
  testthat::expect_equivalent(
    estimate(a, diff)$coefmat,
    e1$coefmat[3, , drop = FALSE]
  )
})
...
test_that("summary.estimate compared with estimate", {
  ...
  expect_true(
    abs(pchisq(q, df = df, lower.tail = FALSE) - e1a$compare$p.value) < 1e-16
  )
  expect_true(
    abs(pchisq(q, df = df, lower.tail = FALSE) - e1b$compare$p.value) < 1e-16
  )
  ...
  e2b <- summary(
    estimate(a3d, function(p) c(p[1] - p[2], p[2] - p[3], p[1] - p[3])),
    null = null
  )
  ...
  expect_true(
    abs(pchisq(q, df = df, lower.tail = FALSE) - e2a$compare$p.value) < 1e-16
  )
  expect_true(
    abs(pchisq(q, df = df, lower.tail = FALSE) - e2b$compare$p.value) < 1e-16
  )
  ...
  expect_true(
    abs(pchisq(q, df = df, lower.tail = FALSE) - e3$compare$p.value) < 1e-16
  )
