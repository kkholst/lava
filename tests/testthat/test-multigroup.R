...
test_that("Multiple group II", {
  ...
  testthat::expect_true(
    mean((coef(e, type = 2)[[1]][1:2, 1] - coef(lm(y ~ x, cbind(d, d))))^2) <
      1e-9
  )
})
...
test_that("Multiple group, constraints", {
  ...
  d <- cbind(d1, d2)
  names(d) <- c(paste0(names(d1), 1), paste0(names(d1), 2))
  e <- estimate(m, d)
