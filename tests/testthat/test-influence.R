...
test_that("negative binomial regression (glm.nb)", {
  ...
  if (requireNamespace("MASS", quietly = TRUE)) {
    ...
    p <- coef(m) + 1
    u1 <- as.vector(numDeriv::jacobian(
      function(p) lava:::logL.glm(m, p = p),
      p
    ))
    u2 <- score(m, p = p)
...
test_that("back.transform", {
  tmp <- summary(
    estimate(coef = 1, IC = c(1, -1, 0), id = 1:3, f = log),
    transform = exp
  )
  tmp2 <- summary(
    estimate(coef = 2, IC = c(0, 1, -1), id = 1:3, f = exp),
    transform = log
  )
...
test_that("IC.numeric", {
  ...
  expect_equal(
    ...
    unname(attr(res, "coef")[3])
  )
  expect_equal(
    var(x) * (length(x) - 1) / length(x),
    unname(attr(res, "coef")[5])
  )
})
