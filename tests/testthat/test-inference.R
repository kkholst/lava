...
test_that("Effects", {
  ...
  suppressWarnings(
    e <- estimate(m, d, control = list(iter.max = 0, start = start))
  )
  ...
  testthat::expect_equivalent(confint(ef)["Direct", ], confint(e)["y1~x", ])
...
test_that("glm-estimator", {
  ...
  testthat::expect_true(
    mean(
      (colSums(score(e, p = coef(e) + 1, indiv = TRUE)) -
        score(e, p = coef(e) + 1, indiv = FALSE))**2
    ) <
      1e-9
  )
})
...
test_that("gaussian", {
  ...
  f <- function(p) {
    lava:::gaussian_objective.lvm(p, x = m, S = S, mu = mu, n = nrow(d))
  }
  g <- function(p) {
    lava:::gaussian_score.lvm(p, x = m, n = nrow(d), data = d, indiv = TRUE)
  }
...
test_that("equivalence", {
  ...
  testthat::expect_output(
    print(eq),
    paste0("y1", lava.options()$symbol[2], "y3")
  )
...
test_that("Bootstrap", {
  ...
  dm <- capture.output(
    B3 <- bootstrap(e, R = 2, fun = function(x) coef(x)[2]^2 + 10)
  )
...
test_that("zero-inflated binomial regression (zib)", {
  ...
  testthat::expect_equivalent(logLik(e, data = d), logLik(e))
  testthat::expect_equivalent(
    vcov(e),
    Inverse(information(e, type = "obs", data = d))
  )
...
if (
  requireNamespace("nlme", quietly = TRUE) &&
    requireNamespace("mets", quietly = TRUE)
) {
  test_that("Prediction with missing data, random intercept", {
    ...
    testthat::expect_output(
      print(summary(e0$estimate)),
      paste0("observations = ", nrow(d0))
    )
  })
}
  ...
  test_that("Random slope model", {
    ...
    system.time(
      e <- estimate(
        ...
        d,
        param = "none",
        control = list(trace = 0, constrain = FALSE)
      )
    )
    ...
    varcomp.nam <- c(
      paste0("u", lava.options()$symbol[2], "u"),
      paste0("s", lava.options()$symbol[2], "s")
    )
    testthat::expect_true(
      mean(diag(sl$varcomp) - coef(e)[varcomp.nam])^2 < 1e-5
    )
...
test_that("Predictions, jacobians", {
  m <- lvm(c(x1, x2, x3) ~ u1, u1 ~ z, c(y1, y2, y3) ~ u2, u2 ~ u1 + z)
  latent(m) <- ~ u1 + u2
  ...
  testthat::expect_true(
    sum(abs(
      numDeriv::jacobian(f, coef(object)) - predictlvm(object)$mean.jacobian
    )) <
      1e-6
  )
  testthat::expect_true(
    sum(abs(
      numDeriv::jacobian(
        function(p) predictlvm(object, p = p)$var,
        coef(object)
      ) -
        predictlvm(object)$var.jacobian
    )) <
      1e-6
  )
  ...
  testthat::expect_true(
    sum(abs(
      numDeriv::jacobian(function(p) f(p, x1 ~ 1), coef(object)) -
  ...
  testthat::expect_true(
    sum(abs(
      numDeriv::jacobian(function(p) f(p, u1 ~ x1 + x2 + x3), coef(object)) -
...
if (requireNamespace("mets", quietly = TRUE)) {
  test_that("multinomial", {
    ...
    m <- lvm()
    covariance(m, pairwise = TRUE) <- ~ y1 + y2 + y3 + y4
    d <- transform(
      sim(m, 5e2),
    ...
    testthat::expect_equivalent(
      K1$coef,
      sum(diag(P) - marg1 * marg2) / (1 - sum(marg1 * marg2))
    )
...
test_that("predict,residuals", {
  ...
  testthat::expect_true(
    mean(var(residuals(e3, std = TRUE))[1] * 99 / 100 - 1) < 1e-3
  )
