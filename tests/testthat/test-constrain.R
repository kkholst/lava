...
test_that("Simple linear constraint", {
  ...
  testthat::expect_true(
    (constraints(e1)[1] - coef(lm(y ~ x + z, d1))["z"])^2 < 1e-9
  )
  s <- summary(e1)
...
test_that("Non-linear in exogenous variables", {
  ...
  testthat::expect_true(
    mean(coef(lm(y ~ x, d)) - coef(e)[c("a", "b")])^2 < 1e-3
  )
})
...
if (lava:::versioncheck('mets', c(1, 0))) {
  test_that("Probit constraints", {
    x <- transform(
      ...
      X1 = as.numeric(cut(X1, breaks = 3)) - 1,
      X2 = as.numeric(cut(X2, breaks = 3)) - 1
    )
...
test_that("Multiple group constraints II", {
  ...
  e <- estimate(
    l,
    subset(twinwide, zyg.1 == "MZ"),
    control = list(method = "NR")
  )
  B <- cbind(1, -1)
  colnames(B) <- c("bw.1,bw.1", "bw.2,bw.2")
  ...
  ee <- estimate(
    list(MZ = l, DZ = l2),
    list(MZ, DZ),
    control = list(method = "NR", tol = 1e-9, constrain = FALSE)
  )
