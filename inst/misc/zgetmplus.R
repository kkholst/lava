...
`findfileseg` <-
  function(infile, startstring, endstring, nlines) {
    ...
    mycmd1 <- paste0("grep -n \"", startstring, "\" ", infile)
    a1 <- system(mycmd1, intern = TRUE)
    if (length(a1) > 0) {
    ...
    if (missing(endstring)) {
      ...
      mycmd2 <- paste0("grep -n \"", endstring, "\" ", infile)
      a2 <- system(mycmd2, intern = TRUE)
      if (length(a2) > 0) {
...
`toMplus.data.frame` <-
  function(
    ...
    for (i in seq_len(ngroups)) {
      newline <- c(
        "\t",
        colnames(x)[((i - 1) * 4 + 1):min(ncol(x), (i * 4))],
        "\n"
      )
    ...
    if (!missing(group)) {
      ...
      mygroupdef <- paste(
        "(",
        paste(groups, groups, sep = "=", collapse = ","),
        ")"
      )
    ...
    if (!techout) {
      ...
      cat(
        file = zz,
        "OUTPUT: MODINDICES(0); TECH1; TECH2; TECH5; STANDARDIZED;\n"
      )
    }
    ...
    cat(
      file = zz,
      "!\tH1TECH3;          !Estimated covar,corr matrices for par. estimates\n"
    )
    cat(file = zz, "!\tPATTERNS;         !Summary of missing data patterns\n")
    cat(
      file = zz,
      "!\tFSCOEFFICIENT;    !Factor score coefficients and posterior covar matrix\n"
    )
    cat(
      file = zz,
      "!\tFSDERTERMINACY;   !Factor score determinacy for each factor\n"
    )
    cat(
      file = zz,
      "!\tTECH1;            !Parameter specifications and starting values\n"
    )
    cat(file = zz, "!\tTECH2             !Parameter derivatives;\n")
    cat(
      file = zz,
      "!\tTECH3;            !Covar and Corr matrices for estimates\n"
    )
    cat(
      file = zz,
      "!\tTECH4;            !Estimated means and covar for the latent variables\n"
    )
    cat(file = zz, "!\tTECH5;            !Optimization matrix\n")
    cat(
      file = zz,
      "!\tTECH6;            !Optimization for categorical variables\n"
    )
...
`toMplus.lvmfit` <-
  function(
    ...
    data = model.frame(x),
    run = TRUE,
    categorical = NULL, ##binary(Model(x)),
    mplusfile = "template.mplus",
    ...
  ) {
    mymodel <- ""
