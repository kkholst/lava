...
#+BEGIN_SRC R :exports both :wrap example
  ...
  m1 <- lvm(c(y1[0:s],y2[0:s],y3[0:s])~1*eta, eta~x) 
  e1 <- estimate(m1,d)

  pp <- function(mu,var,data,...) cbind(eta=mu[,"eta"],eta2=mu[,"eta"]^2+var["eta","eta"])
  ee <- measurement.error(e1, z1~1+x, data=d, predictfun=pp)
  ee
#+END_SRC
...
(Intercept)  0.000138  0.2212 -0.433 0.434 1.00e+00
x            0.854841  0.3348  0.199 1.511 1.07e-02
eta          1.148610  0.2221  0.713 1.584 2.31e-07
eta2        -0.032049  0.0485 -0.127 0.063 5.09e-01
#+END_example

#+BEGIN_SRC R :exports both :results output graphics :file inst/me1.png  
  f <- function(p) p[1]+p["eta"]*eta+p["eta2"]*eta^2
  eta <- seq(-1,1,length.out=100)
  plot(ee, f, data=data.frame(eta))
#+END_SRC
